...
≤ 90%), lower-secondary completion at T predicts outcome at T+lag.
T = 1960..1990 (5yr). Outcome-specific lag: TFR uses LAG_TFR=5
(biological parent-of-newborn timing); U5MR uses LAG_CHILDREARING=12
(childrearing window); LE, log GDP use LAG_GENERATION=28
(cross-generation / time-to-agency).
...
# U5MR uses the childrearing window (LAG_CHILDREARING=12); LE, log GDP use
# the cross-generation lag (LAG_GENERATION=28).
# Column-name suffix encodes the per-outcome lag (tp<lag>): LE/log GDP use the
# cross-generation lag (tp28), TFR the biological-parent lag (tp5), U5MR the
# childrearing-window lag (tp12).
OUTCOME_LAG = {
    "le_tp28":      LAG_GENERATION,
    "tfr_tp5":      LAG_TFR,
    "log_gdp_tp28": LAG_GENERATION,
...
OUTCOMES = [
    ("life_expectancy", "le_tp28", "life_expectancy_years.csv", "linear"),
    ("tfr", "tfr_tp5", "children_per_woman_total_fertility.csv", "linear"),
    ("log_gdp", "log_gdp_tp28", "gdppercapita_us_inflation_adjusted.csv", "log"),
    ("u5mr", "u5mr_tp12", "child_mortality_u5.csv", "linear"),
]


def build_panel(edu_annual, outcome_df, ycol, ytransform, ssa_only):
    """Long panel rows = country × T."""
    lag = OUTCOME_LAG[ycol]
    rows = []
...
print("=" * 82)
print("SSA-SUBSAMPLE β FOR HANDOUT — lower-sec completion at T → outcome at T+lag")
print(f"Country FE, country-clustered SE; entry≥{ENTRY}%, ceiling≤{CEILING}%; "
      f"lag: TFR={LAG_TFR}, U5MR={LAG_CHILDREARING}, LE/logGDP={LAG_GENERATION}.")
print("=" * 82)
...
print("\nNotes:")
print("  • Two specs reported. 'pure FE': LE(T+28) ~ edu(T) + α_i. '+init Y':")
print("    LE(T+28) ~ edu(T) + LE(T) + α_i. The handout uses '+init Y' (the")
...
write_checkin(
    "handout_ssa_betas.json",
    {
        "method": (
            ...
            f"(entry≥{ENTRY}%, ceiling≤{CEILING}%); lower-sec completion at "
            f"T → outcome at T+lag; T=1960-1990 (5yr). Outcome-specific lag: "
            f"TFR uses LAG_TFR={LAG_TFR} (biological parent timing); "
            f"U5MR uses LAG_CHILDREARING={LAG_CHILDREARING} (childrearing "
            f"window); LE, log GDP use LAG_GENERATION={LAG_GENERATION} "
            "(time-to-agency). SSA = UN M49 (48 countries). "
            "Outcomes: LE, TFR, log GDP per capita (constant 2015 USD), U5MR."
        ),
        "spec": {
            "outcome_lag": OUTCOME_LAG, "entry": ENTRY, "ceiling": CEILING,
