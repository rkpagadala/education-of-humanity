...
Life-history label (§the-generational-lag):
  Predictors: WCDE v3 vs Barro-Lee v3 measures of 20-24 cohort
  completion at T. Outcome: phenotype at its lag (LE at T+28, TFR at
  T+5, U5MR at T+12).
  Generational relation: parent → child (cross-generation step).
  Use: data-source identification — picks the WCDE measure based on
  better predictive fit and phenotype-consistency. The USSR republics
  fail phenotype-consistency under WCDE (Goskomstat over-reporting),
  driving the USSR exclusion in §hollow-education.
...
attainment — better predicts the downstream phenotype 28 years later?
...
---------------
 (1) Build a panel on overlapping years {1960, 1970, 1980, 1990}
     with WCDE lower-secondary completion (age 20-24),
     B-L ls+lh (reached secondary+, age 25-34), and
     B-L yr_sch (mean years of schooling, age 25-34).
 (2) For each outcome at its lag (LE at T+28, TFR at T+5, log U5MR at
     T+12), run three OLS specifications with country+year FEs:
       (a) outcome(T+lag) ~ WCDE(T)
       (b) outcome(T+lag) ~ BL_ls_lh(T)
       (c) outcome(T+lag) ~ BL_yr_sch(T)
...
from _shared import DATA, PROC, load_wide_indicator, write_checkin, LAG_GENERATION, LAG_CHILDREARING, LAG_TFR

YEARS_EDU = [1960, 1970, 1980, 1990]  # years we observe education
# Outcome-specific forward lag. TFR uses LAG_TFR=5 (biological parent-of-
# newborn timing); U5MR uses LAG_CHILDREARING=12 (childrearing window); LE
# uses LAG_GENERATION=28 (time-to-agency).
OUTCOME_LAG = {
    "tfr":      LAG_TFR,
    "le":       LAG_GENERATION,
...
def load_panel():
    ...
    WCDE lsec, B-L ls+lh, B-L yr_sch, and TFR/LE/U5MR at their lags
    (LE at edu_year+28, TFR at edu_year+5, U5MR at edu_year+12)."""
    ...
    for _, r in merged.iterrows():
        ...
        # Per-outcome year. TFR at edu_year+LAG_TFR; U5MR at
        # edu_year+LAG_CHILDREARING; LE at edu_year+LAG_GENERATION.
        tfr_year = r['year'] + OUTCOME_LAG['tfr']
        le_year  = r['year'] + OUTCOME_LAG['le']
        u5_year  = r['year'] + OUTCOME_LAG['u5mr']
        if str(le_year) not in le.columns:
            continue
        try:
            tv = (tfr.loc[wn, str(tfr_year)]
                  if wn in tfr.index and str(tfr_year) in tfr.columns else np.nan)
            lv = (le.loc[wn, str(le_year)]
                  if wn in le.index and str(le_year) in le.columns else np.nan)
            uv = (u5.loc[wn, str(u5_year)]
                  if wn in u5.index and str(u5_year) in u5.columns else np.nan)
        except KeyError:
            continue
        outs.append({
            'country': r['country'],
            'edu_year': r['year'],
            # out_year reports the LE year (T+LAG_GENERATION). The TFR
            # observation is read at T+LAG_TFR=5 and the U5MR observation at
            # T+LAG_CHILDREARING=12; both read separately via OUTCOME_LAG.
            'out_year': le_year,
...
def main():
    ...
    # ── HEAD-TO-HEAD FIT ──────────────────────────────────────
    outcomes = {'tfr': f'TFR (T+{OUTCOME_LAG["tfr"]})',
                'le': f'Life expectancy (T+{OUTCOME_LAG["le"]})',
                'log_u5mr': f'log U5MR (T+{OUTCOME_LAG["log_u5mr"]})'}
