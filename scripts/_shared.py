...
import pandas as pd
import numpy as np
# statsmodels is imported lazily inside the one regression helper that needs
# it (see fixed_effects_ols below). Keeping it out of module load lets the
# country-name canonicalization (standardize_country_name) be imported in
# thin environments — e.g. the website chatbot container — that ship pandas
# and numpy but not statsmodels.
...
# ── Web figure mode ───────────────────────────────────────────────────
# FOR_WEB=1  → titled versions for website, English (paper/figures_web/)
# FOR_ZH=1   → titled versions for the Chinese (/zh) website pages; same
#              layout as FOR_WEB but every baked-in string is Chinese and the
#              file is written with a `_zh` suffix (paper/figures_web/<name>_zh.png)
#              so it rides the existing figures_web → public sync.
# default    → paper-mode figures without in-image titles (paper/figures/)
FOR_WEB    = os.environ.get("FOR_WEB", "0") == "1"
FOR_ZH     = os.environ.get("FOR_ZH", "0") == "1"
PAPER_FIGS = os.path.join(REPO_ROOT, "paper", "figures")
WEB_FIGS   = os.path.join(REPO_ROOT, "paper", "figures_web")
if FOR_ZH:
    FOR_WEB = True            # reuse the titled-mode layout in every figure script
if FOR_WEB:
    os.makedirs(WEB_FIGS, exist_ok=True)
if FOR_ZH:
    # Register a CJK font so matplotlib renders Simplified Chinese. No dedicated
    # SC face ships here, but the Noto CJK collection's Han glyphs cover it; the
    # collection registers under the "Noto Sans CJK JP" name.
    import matplotlib
...
def L(en, zh):
    """Return the Chinese string when building /zh figures, else the English one."""
    return zh if FOR_ZH else en


def fig_file(name):
    """Insert a `_zh` suffix before the extension when building Chinese figures."""
    if FOR_ZH:
...
# ── Lag constants: education year T → outcome year ────────────────
# Education is for the cohort age 20-24 in year T (WCDE convention).
# The right lag depends on the OUTCOME, because biology and "time-to-
# agency" run on different clocks.
#
# BIOLOGICAL — cohort is itself the parent of the children whose
# outcome is observed:
#   LAG_TFR = 5
#     Cohort age 20-24 at T is age 25-29 at T+5 — reproductive peak.
#     Period TFR at T+5 reflects this cohort's own childbearing.
#   LAG_CHILDREARING = 12
#     The cohort's own children (born ~T+5) are at risk of under-5
#     mortality through ~T+10. Midpoint ≈ 10-15. Life expectancy at
#     birth routes here too: it is mechanically dominated by child
#     mortality (within-country, log U5MR alone carries ~0.70 of LE's
#     variation; child + adult mortality together ~0.90), so the
#     schooling that moves LE is the same childrearing-window schooling
#     that moves U5MR, not a generation-back signal.
#
# EDU → EDU transmission — literal one-generation step:
#   LAG_GENERATION = 28
#     Parent cohort at T → child cohort completing schooling at T+28.
#     Mean generation length = mean age at childbearing (MAC ≈ 28.7,
#     UN WPP 2024; see mean_age_first_birth_invariance.py). This is the
#     cohort-replacement interval, NOT time-to-agency. A child reaches
#     agency in ~18-20 years; that short interval is already carried by
#     the outcome lags above (LAG_TFR=5, LAG_CHILDREARING=12) — the cohort
#     we measure (age 20-24) is already adult, so its own childbearing
#     and childrearing effects land within 5-12 years.
#
# THREE-GENERATION — pair the appropriate parent lag with one extra
# LAG_GENERATION for the grandparent of the parent cohort. The TFR
# grandparent lag is LAG_TFR + LAG_GENERATION = 33; the edu→edu and
# childrearing grandparent depth is two generation steps,
# LAG_GENERATION + LAG_GENERATION = LAG_TWO_GENERATIONS = 56.
#   LAG_TWO_GENERATIONS = 56
#
# Contemporaneous (no lag, robustness only — not a headline spec):
#   LAG_CONTEMPORANEOUS = 0
LAG_CONTEMPORANEOUS = 0       # same-year (robustness only)
LAG_TFR             = 5       # cohort → own newborn (biological parent timing)
LAG_CHILDREARING    = 12      # cohort → own children's U5MR (~10-15); LE at birth shares this window (U5MR-dominated)
LAG_LE              = 12      # LE at birth is mortality-dominated (mostly U5MR) → childrearing window, not a generation
LAG_GENERATION      = 28      # edu→edu one-step (mean generation length, MAC≈28.7, UN WPP 2024)
LAG_TWO_GENERATIONS = 2 * LAG_GENERATION  # two generation steps back (grandparent depth)
...
WB_TO_WCDE = {
    ...
    "eswatini": "swaziland",
    "timor-leste": "timor-leste",
    # WB outcome CSVs and WCDE files both spell these as below; without these
    # bridges a forward WB->WCDE lookup misses (the paper scripts also resolve
    # them via the reverse-map identity fallback, but the explicit map is correct
    # and lets downstream tools share one source of truth).
    "tanzania": "united republic of tanzania",
...
def write_checkin(filename, data, script_path=None, base_dir=None):
    """Write a checkin JSON file. Adds a 'script' path.

    base_dir defaults to the repo-level CHECKIN directory. ML pipelines
    pass base_dir=ML_CHECKIN so their results pool under scripts/ml/checkin/.

    No production date is stamped into the JSON: the file's last-modified
    time is the source of truth for when a result was produced. Stamping a
    date churned every checkin on every `make verify` run even when the
    numbers were identical.
    """
    if script_path:
        data["script"] = script_path
    target_dir = base_dir if base_dir is not None else CHECKIN
