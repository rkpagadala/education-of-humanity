...
Life-history label (§the-generational-lag):
  Predictor: country crosses threshold x of 20-24 cohort lower-sec
  completion (x ∈ {5%, 10%, 15%, 20%}). Outcome: child education
  stock at T+28 (event-time +28). Cross-generation step.
  Use: threshold-sensitivity of the compounding-generations signature.

Sweep the entry threshold for the Callaway & Sant'Anna (2021) estimator
of education -> child education at T+28.
...
Method:
  ...
  over post-treatment (g,t) pairs (simple average), and report:
    ...
    - Aggregate ATT (child education at T+28)
...
from _shared import (PROC, REGIONS, write_checkin, load_wb, get_wb_val, LAG_GENERATION,
...
print("\nThreshold sweep -- aggregate ATT (child education at T+28)")
print("=" * 70)
...
write_checkin(
    "cs_threshold_robustness.json",
    {
        "method": (
            ...
            "T+28, swept across entry thresholds 5-15%. Same panel as "
