...
#
# Test equation:
#   child_t = beta * parent_{t-28} + gamma * parent_{t-28+5} + alpha_i + u_t
#   H0: gamma = 0  (strict exogeneity)
#
#   parent_{t-28+5} is the lead of the regressor: the parental education
...
def load_panel():
    """Panel with parent_{t-28}, child_t, and the one-period lead parent_{t-23}."""
    ...
    # Parent lag (28) and lead (-23) are off the 5-year WCDE grid; interpolate to
    # annual so those lookups resolve (child cohorts stay on the 5-year grid).
...
def main():
    ...
    write_checkin("wooldridge_exogeneity.json", {
        "notes": (
            ...
            "parent_{t-28}) to the FE regression; test H0: γ=0. "
