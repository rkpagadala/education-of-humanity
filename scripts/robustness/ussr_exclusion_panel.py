...
Data sources (same as panel_expansion_fe.py):
...
from _shared import (PROC, DATA, REGIONS as NON_SOVEREIGN,
                     ...
                     wcde_to_wdi, LAG_GENERATION, LAG_TFR,
                     interpolate_wide_to_annual)

PARENTAL_LAG = LAG_GENERATION
# Per-outcome forward lag. TFR uses LAG_TFR=5 (biological parent-of-newborn
# timing — cohort aged 20-24 at T is at reproductive peak at T+5); LE and
# U5MR use LAG_GENERATION=28 (time-to-agency).
OUTCOME_LAG = {
    "edu":  LAG_GENERATION,
    "le":   LAG_GENERATION,
    "tfr":  LAG_TFR,
...
ENTRY_COHORT_LOWER = 10.0  # expansion-sample window lower bound
ENTRY_COHORT_UPPER = 90.0  # expansion-sample window upper bound
...
def build_edu_panel(edu_df, exclude=None):
    """Build (country, year) rows with child lower-secondary completion at
    year Y and parent lower-secondary completion at year Y-25. Mirrors
    panel_expansion_fe.py."""
    ...
    # Expansion-sample filter (child-year lower-sec in [10, 90])
    if len(df):
        df = df[(df["child"] >= ENTRY_COHORT_LOWER) &
                (df["child"] <= ENTRY_COHORT_UPPER)].reset_index(drop=True)
    return df


def build_outcome_panel(edu_df, outcome_df, exclude=None, lag=PARENTAL_LAG):
    """Build (country, year) rows pairing education at year T with outcome
    at year T+lag. The entry-cohort filter uses child-year lower-sec at
    year T+LAG_GENERATION (the time-to-agency anchor) so that the sample
    window is identical across outcomes regardless of the outcome's own
    lag. Country names are lowercased for WDI join via wcde_to_wdi()."""
    exclude = exclude or set()
    rows = []
    for country in edu_df.index:
        if country in NON_SOVEREIGN or country in exclude:
        ...
        if wdi_name not in outcome_df.index:
            continue
        for y in OUTCOME_YEARS:
            t_year = y - PARENTAL_LAG          # base year (predictor anchor)
            out_year = t_year + lag            # outcome year for this spec
            sy_t, sy_out = str(t_year), str(out_year)
            sy_child = str(y)                  # child-year anchor for sample filter
            if sy_t not in edu_df.columns or sy_child not in edu_df.columns:
                continue
            if sy_out not in outcome_df.columns:
                continue
            edu_val = edu_df.loc[country, sy_t]
            child_edu = edu_df.loc[country, sy_child]
            out_val = outcome_df.loc[wdi_name, sy_out]
            if np.isnan(edu_val) or np.isnan(out_val) or np.isnan(child_edu):
                continue
            rows.append({
                "country": country,
                "year": y,
    ...
    # Expansion-sample filter (child-year lower-sec in [10, 90])
    if len(df):
        df = df[(df["child_edu"] >= ENTRY_COHORT_LOWER) &
                (df["child_edu"] <= ENTRY_COHORT_UPPER)].reset_index(drop=True)
    return df
...
def main():
    edu = pd.read_csv(os.path.join(PROC, "lower_sec_both.csv"),
                      index_col="country")
    # Education lagged lookups are at y - LAG_GENERATION (28), off the 5-year grid;
    # interpolate to annual so lagged lookups resolve (on-grid at the old 25). Keep
    # string columns so the str(year) lookups below are unchanged.
    edu.columns = edu.columns.astype(int)
    edu = interpolate_wide_to_annual(edu)
    ...
    # ── LE 25-yr forward FE ─────────────────────────────────────────
    print("\nLE(T) ~ lsec(T-28)  (country FE)")
    le_full = build_outcome_panel(edu, le, exclude=set(), lag=OUTCOME_LAG["le"])
    le_clean = build_outcome_panel(edu, le, exclude=USSR_WCDE, lag=OUTCOME_LAG["le"])
    ...
    # ── TFR forward FE (lag = LAG_TFR = 5) ──────────────────────────
    print(f"\nTFR(T) ~ lsec(T-{PARENTAL_LAG})  (country FE; TFR read at "
          f"T+LAG_TFR={OUTCOME_LAG['tfr']} — biological parent timing)")
    tfr_full = build_outcome_panel(edu, tfr, exclude=set(), lag=OUTCOME_LAG["tfr"])
    tfr_clean = build_outcome_panel(edu, tfr, exclude=USSR_WCDE, lag=OUTCOME_LAG["tfr"])
    ...
    print("\nlog U5MR(T) ~ lsec(T-28)  (country FE)")
    u5_log = u5.copy().clip(lower=0.1)
    u5_log = np.log(u5_log.astype(float))
    u5_full = build_outcome_panel(edu, u5_log, exclude=set(), lag=OUTCOME_LAG["u5"])
    u5_clean = build_outcome_panel(edu, u5_log, exclude=USSR_WCDE, lag=OUTCOME_LAG["u5"])
