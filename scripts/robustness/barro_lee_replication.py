...
#   Predictor: Barro-Lee mean years of schooling at T. Outcomes at
#   T+lag, where lag is outcome-specific:
#     - LE  → LAG_GENERATION = 28 (time-to-agency)
#     - TFR → LAG_TFR        =  5 (biological parent timing — cohort
#       age 20-24 at T is age 25-29 at T+5, reproductive peak)
#   Use: measure robustness — replicates forward-prediction
#   regressions with an alternative data source (BL instead of WCDE).
#
# Produces:
...
at T+lag) using Barro-Lee data. Lag is outcome-specific: LE uses
LAG_GENERATION=28 (time-to-agency), TFR uses LAG_TFR=5 (biological
parent timing). Show the result holds on an independent education
dataset with different methodology, and on a restricted post-1970
...
from _shared import (
    PROC, DATA, CHECKIN, REGIONS, NAME_MAP, write_checkin,
    LAG_TFR, LAG_GENERATION,
)

# Per-outcome lag. Column names like le_t25 / tfr_t25 are KEPT for
# downstream compatibility even though the underlying TFR lag is now
# LAG_TFR=5 (biological parent timing), not 25.
OUTCOME_LAG = {
    "le_t25":  LAG_GENERATION,  # life expectancy (time-to-agency)
    "tfr_t25": LAG_TFR,         # column name kept for compatibility; underlying lag is LAG_TFR=5
}
...
T_YEARS_BL = [1960, 1965, 1970, 1975, 1980, 1985, 1990]
# LE outcome years span T+25 = 1985..2015; TFR outcome years span
# T+5 = 1965..1995 with the biological parent timing.
...
for _, row in bl.iterrows():
    ...
    le_val = np.nan
    tfr_val = np.nan
    # Per-outcome lag: LE uses LAG_GENERATION=28, TFR uses LAG_TFR=5.
    le_yr_str = str(t + OUTCOME_LAG["le_t25"])
    tfr_yr_str = str(t + OUTCOME_LAG["tfr_t25"])
    if wb_name in le_wb.index and le_yr_str in le_wb.columns:
        le_val = le_wb.loc[wb_name, le_yr_str]
        le_val = float(le_val) if pd.notna(le_val) else np.nan
    if wb_name in tfr_wb.index and tfr_yr_str in tfr_wb.columns:
        tfr_val = tfr_wb.loc[wb_name, tfr_yr_str]
    ...
    rows_bl.append({
        ...
        # Column names kept for compatibility; tfr_t25 carries values at
        # T+LAG_TFR=5, not T+25.
...
for _, row in edu_wcde.iterrows():
    ...
    le_val = np.nan
    tfr_val = np.nan
    # Per-outcome lag: LE → LAG_GENERATION=28, TFR → LAG_TFR=5.
    le_yr_str = str(t + OUTCOME_LAG["le_t25"])
    tfr_yr_str = str(t + OUTCOME_LAG["tfr_t25"])
    if wb_name in le_wb.index and le_yr_str in le_wb.columns:
        le_val = le_wb.loc[wb_name, le_yr_str]
...
# ── Checkin ──────────────────────────────────────────────────────────────
checkin = {
    "test": "Barro-Lee replication of forward-prediction and residualization",
    "method": (
        "Per-outcome lag: LE uses LAG_GENERATION=28 (time-to-agency); "
        "TFR uses LAG_TFR=5 (biological parent-of-newborn timing). "
        "Column names le_t25/tfr_t25 retained for downstream compatibility."
    ),
