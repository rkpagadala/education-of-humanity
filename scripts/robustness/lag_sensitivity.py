...
Life-history label (§the-generational-lag):
  Predictor: WCDE 20-24 cohort completion at T-L for L ∈ {10,12,15,20,25,30}.
  Outcomes: TFR/LE/U5MR/childEdu at T. Sensitivity sweep spanning the
  childrearing window (LE/U5MR canonical lag, ~10-15) through the
  28-year cross-generation step. Use: confirm residualized GDP
  near-zero result holds across the full biologically plausible lag
  range, not just at one specific lag. Companion to
  robustness/residualization_by_lag.py (wider lag range 0-50).

Does the zero GDP result hold at different lags?

Runs the residualized analysis at lags 10, 12, 15, 20, 25, 30 for all four
outcomes (LE, TFR, U5MR, child education). Lower secondary, entry=10%.

If the result holds across lags, it's not an artifact of the 28-year choice.
"""
...
write_checkin("lag_sensitivity.json", {
    "method": "Country FE, residualized GDP. Lags 10/12/15/20/25/30. Lower secondary, entry=10%. Four outcomes.",
    "results": all_results,
