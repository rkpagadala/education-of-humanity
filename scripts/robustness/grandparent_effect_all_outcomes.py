...
Life-history label (§the-generational-lag):
  Per-outcome biological lag stack. For TFR: parent at T-5 (cohort age
  20-24 → newborn at T) and grandparent at T-33. For U5MR: parent at
  T-12 (childrearing window) and grandparent at T-40. For LE/child-edu:
  parent at T-28 (one generation) and grandparent at T-56. Two-step vs
  one-step cross-generation test across all four outcomes. Largest
  grandparent independent effect is on child survival (log U5MR).
  Use: kin-radius transmission identification.

Generalization of grandparent_effect.py to all four development outcomes.

Test whether grandparent-generation education independently predicts each
development outcome, controlling for parent-generation education. Lags are
chosen biologically per-outcome (see OUTCOME_LAGS below).

Outcomes covered:
  - Life expectancy (LE)            -- WDI
  - Total fertility rate (TFR)      -- WDI
  - Under-5 mortality, log (U5M)    -- WDI (log-transformed)
  - Child education (child_edu)     -- WCDE (education at T+28)

Methodology mirrors grandparent_effect.py:
  - TFR: parent at T-LAG_TFR (5), grandparent at T-LAG_TFR-LAG_GENERATION (33)
  - U5MR: parent at T-LAG_CHILDREARING (12),
    grandparent at T-LAG_CHILDREARING-LAG_GENERATION (40)
  - LE / child_edu: parent at T-LAG_GENERATION (28),
    grandparent at T-LAG_TWO_GENERATIONS (56)
  - Outcome at T (for LE/TFR/U5M), or grandchild education at T+28.
...
from _shared import (
    PROC, DATA, CHECKIN, REGIONS, NAME_MAP, WB_REGIONS,
    LAG_TFR, LAG_CHILDREARING, LAG_GENERATION, LAG_TWO_GENERATIONS,
)

# Per-outcome biological lag stack. TFR uses parent-of-newborn timing
# (LAG_TFR=5) and grandparent one generation back (LAG_TFR+LAG_GENERATION=33).
# U5MR uses the childrearing window (LAG_CHILDREARING=12) and grandparent one
# generation back (LAG_CHILDREARING+LAG_GENERATION=40). LE/child-edu keep the
# time-to-agency cohort framing (28/56).
OUTCOME_LAGS = {
    "tfr":       (LAG_TFR,        LAG_TFR + LAG_GENERATION),
    "tfr_log":   (LAG_TFR,        LAG_TFR + LAG_GENERATION),
    "le":        (LAG_GENERATION, LAG_TWO_GENERATIONS),
    "le_log":    (LAG_GENERATION, LAG_TWO_GENERATIONS),
    "u5_log":    (LAG_CHILDREARING, LAG_CHILDREARING + LAG_GENERATION),
    "child_edu": (LAG_GENERATION, LAG_TWO_GENERATIONS),
}
...
# Off-grid lags (parent 28, grandparent 56, etc.) fall between 5-year cohort
# columns; interpolate the year columns to annual so lagged lookups resolve.
# Parent-anchor iteration in the builders stays on the 5-year grid (int(c)%5==0),
# so sample density is unchanged — only the off-grid lookups are interpolated.
...
edu = edu[["country", "country_lc"]].join(_w, on="country")
...
# ── Panel builders ────────────────────────────────────────────────────
def build_panel(edu_df, outcome_df, outcome_name, lag_gm=LAG_TWO_GENERATIONS, lag_m=LAG_GENERATION,
                log_outcome=False):
...
def build_edu_panel(edu_df, lag_gm=LAG_TWO_GENERATIONS, lag_m=LAG_GENERATION):
    """Build three-generation panel for child education outcome.

    grandparent_edu at T-56, parent_edu at T-28, child_edu at T+28
    (relative to mother year T: GM at T-28, child at T+28 → all from one CSV).
    """
    rows = []
    edu_years = sorted([int(c) for c in edu_df.columns if c.isdigit() and int(c) % 5 == 0])
...
le_m, le_gm = OUTCOME_LAGS["le"]
tfr_m, tfr_gm = OUTCOME_LAGS["tfr"]
u5_m, u5_gm = OUTCOME_LAGS["u5_log"]
ce_m, ce_gm = OUTCOME_LAGS["child_edu"]

panel_le = build_panel(edu, le, "le", lag_m=le_m, lag_gm=le_gm)
panel_tfr = build_panel(edu, tfr, "tfr", lag_m=tfr_m, lag_gm=tfr_gm)
panel_u5 = build_panel(edu, u5, "u5_log", lag_m=u5_m, lag_gm=u5_gm,
                       log_outcome=True)
panel_edu = build_edu_panel(edu, lag_m=ce_m, lag_gm=ce_gm)

# Log variants for LE and TFR — match the log rows of tab:edu-outcomes
# in the paper. Existing level panels are kept so verify regs that point
# at the level keys continue to pass.
panel_le_log = build_panel(edu, le, "le_log", lag_m=le_m, lag_gm=le_gm,
                           log_outcome=True)
panel_tfr_log = build_panel(edu, tfr, "tfr_log", lag_m=tfr_m, lag_gm=tfr_gm,
                            log_outcome=True)
...
all_results = {
    ...
    "le_log":    run_outcome(panel_le_log, "le_log",    "Life expectancy (log LE)"),
    "tfr":       run_outcome(panel_tfr,    "tfr",       "Total fertility rate (TFR)"),
    "tfr_log":   run_outcome(panel_tfr_log,"tfr_log",   "Total fertility rate (log TFR)"),
    "u5_log":    run_outcome(panel_u5,     "u5_log",    "Under-5 mortality (log)"),
    "child_edu": run_outcome(panel_edu,    "child_edu", "Child education (T+28)"),
}
...
output = {
    "method": (
        ...
        "(3) outcome ~ grandparent_edu. Per-outcome biological lag: TFR uses "
        "parent at T-5 (cohort age 20-24 → newborn at T) and grandparent at "
        "T-33; U5MR uses parent at T-12 (childrearing window) and grandparent "
        "at T-40; LE / child-edu use parent at T-28 and grandparent at "
        "T-56. WCDE v3 lower secondary completion, both sexes, ages 20-24. "
