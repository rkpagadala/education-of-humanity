...
PAPER ANCHOR — Chapter 9 §What the Panel Does Not Carry (apparatus pointer)
  Question: why does adding year fixed effects collapse the parent->child
            education coefficient from β ≈ 0.48 (one-way FE) to β ≈ 0.08
            (two-way FE)? Is it a refutation of the prediction, or the
            staggered-adoption pathology?
  Produces: the Goodman-Bacon (2021) decomposition cited inline as the
            apparatus reading behind the Callaway-Sant'Anna fix. Shows the
            TWFE collapse is "already-transitioned countries used as
            controls", not absence of signal.
  Checkin:  checkin/goodman_bacon_decomposition.json

Life-history label (§the-generational-lag):
  Decomposes the TWFE estimator for the education → development
  effect at T+28 (cross-generation step). Use: identification —
  diagnoses where the TWFE coefficient comes from across staggered
  threshold crossings; clean-control comparisons are needed for the
  Callaway-Sant'Anna result to be interpretable.

Goodman-Bacon (2021) decomposition of the two-way fixed effects estimator.

METHOD / DIAGNOSTIC / ESTIMATOR distinction:
  - Method:     difference-in-differences with staggered adoption.
  - Diagnostic: Goodman-Bacon (2021) decomposition — decomposes a TWFE
                estimate into its constituent 2×2 DID comparisons and
                identifies forbidden-comparison weights. NOT an estimator;
                it does not produce an alternative coefficient, it
                attributes the TWFE β to its component comparisons.
  - Estimators: Callaway–Sant'Anna (2021), Sun–Abraham (2021), de
                Chaisemartin–d'Haultfœuille (2020), Borusyak–Spiess–
                Jaravel (2024), Honest DiD (Rambachan–Roth 2023).

Scope of the GB decomposition: GB (2021) is developed for BINARY
staggered treatment designs. The decomposition into 2×2 comparisons
and the negative-weights pathology are binary-specific. This script
applies GB to a binarised discretisation of education (D_it = 1 if
lower-secondary completion ≥ 10%) as a diagnostic. The continuous
education case is handled separately by the de Chaisemartin–
d'Haultfœuille (2020) framework in the econometric battery, not by
GB. Numbers from this script should not be reported as the GB
diagnostic for continuous education.
...
decomposes the TWFE estimator for the binary treatment into its
constituent 2×2 DID comparisons following Goodman-Bacon (2021) to
show WHERE the attenuation comes from.

Framework:
  - Binary treatment: D_it = 1 if country i has ≥ 10% lower secondary
    completion at time t (absorbing: once crossed, stays crossed)
  - Outcome: child lower secondary completion at t+28 (intergenerational)
...
are the forbidden comparisons in the GB sense: using already-treated
units as counterfactuals for newly-treated ones. Clean comparisons
(against genuinely untreated countries) yield β ≈ 11, but carry only
~7% of the total weight. The Callaway-Sant'Anna (2021) estimator
(companion script) fixes this by restricting controls to not-yet-
treated units.
...
References:
  ...
  de Chaisemartin, C. & d'Haultfœuille, X. (2020). Two-Way Fixed
    ...
    Economic Review, 110(9), 2964–96. [continuous-treatment companion
    to GB]
...
from _shared import (PROC, CHECKIN, REGIONS, write_checkin, LAG_GENERATION,
                     interpolate_wide_to_annual, completion_at_year)

# ── Constants ────────────────────────────────────────────────────────
THRESHOLD = 10        # lower secondary completion %
PERIODS = list(range(1950, 1995, 5))   # 9 five-year periods
LAG = LAG_GENERATION  # outcome at T+28
...
def _outcomes_ok(c):
    vals = [completion_at_year(edu_annual, c, t + LAG) for t in PERIODS]
    return all(v is not None and not np.isnan(v) for v in vals)


countries = [c for c in sorted(complete.index) if _outcomes_ok(c)]
N = len(countries)
T = len(PERIODS)
...
for c in countries:
    for t in PERIODS:
        ...
        child_edu = float(completion_at_year(edu_annual, c, t + LAG))
        rows.append({
