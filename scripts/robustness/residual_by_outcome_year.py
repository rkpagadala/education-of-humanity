...
Sweep residualized GDP R² by individual outcome year for ALL four outcomes:
  - Life expectancy            (lag = LAG_GENERATION = 28)
  - Total fertility rate       (lag = LAG_TFR        =  5)
  - Child education            (lag = LAG_GENERATION  = 25)
  - Under-5 mortality          (lag = LAG_CHILDREARING = 12)
...
lower secondary completion, T=1960-1990. TFR uses biological parent
timing (5y); U5MR uses the childrearing window (12y); LE and child
education use time-to-agency / edu→edu (25y).
...
from _shared import write_checkin, fmt_r2, LAG_TFR, LAG_CHILDREARING, LAG_GENERATION

# ── Parameters ────────────────────────────────────────────────────

T_YEARS = list(range(1960, 1995, 5))
# Per-outcome lag. Keys match the `col` field below.
OUTCOME_LAG = {
    "le":        LAG_GENERATION,
    "tfr":       LAG_TFR,
...
# Child education panel: education at T+lag predicted by education at T
child_edu_lag = OUTCOME_LAG["child_edu"]
child_edu_rows = []
for c in sorted(edu_annual.keys()):
    s = edu_annual[c]
    for t in T_YEARS:
        if t not in s.index or (t + child_edu_lag) not in s.index:
            continue
        edu_val = s[t]
        child_val = s[t + child_edu_lag]
...
print("=" * 100)
print(f"Entry >= {ENTRY_THRESHOLD}%, ceiling <= {CEILING}%, T=1960-1990")
print(f"Per-outcome lag: " + ", ".join(f"{k}={v}" for k, v in OUTCOME_LAG.items()))
...
write_checkin("residual_by_outcome_year_all.json", {
    "method": (
        ...
        f"Entry >= {ENTRY_THRESHOLD}%, ceiling <= {CEILING}%, T=1960-1990. "
        f"Per-outcome lag: TFR uses LAG_TFR={LAG_TFR}y (biological "
        f"parent timing); U5MR uses LAG_CHILDREARING={LAG_CHILDREARING}y "
        f"(childrearing window); LE, child education use LAG_GENERATION="
        f"{LAG_GENERATION}y (time-to-agency / edu→edu). "
