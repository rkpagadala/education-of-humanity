...
Life-history label (§the-generational-lag):
  Predictors: educational and institutional measures on AJR's
  selected colony sample. Outcomes: demographic phenotype at T+28
  (cross-generation step). Use: identification — re-analyzes AJR's
  own selected sample with the paper's lens to show settler-mortality
  IV captures Protestant-vs-Catholic schooling tradition, not pure
...
(TFR<3.65 ∧ LE>69.8) via 28-year lag, home-niche mechanism,
...
Hypothesis battery:
  ...
  M1   One-generation first-difference: Δedu_1950→1978 → Δoutcome_1978→2006
       (LAG_GENERATION=28 step; edu_1978 resolves off the 5-year WCDE grid via
       annual interpolation of lsec_both at load)
...
from _shared import (PROC, DATA, REGIONS, load_wb, NAME_MAP, write_checkin,
                     TFR_THRESHOLD, LE_THRESHOLD, LAG_GENERATION)
...
def _interp_panel_to_annual(panel):
    """Interpolate a country-indexed WCDE panel (5-year-grid year columns) to
    annual columns so off-grid generational lags (T+28 under LAG_GENERATION)
    resolve by direct lookup. Mirrors grandparent_effect.py; fills only inside
    each country's observed span (no extrapolation)."""
    yc = sorted([c for c in panel.columns if str(c).isdigit()], key=int)
    w = panel[yc].copy()
    w.columns = [int(c) for c in w.columns]
    w = w.reindex(columns=range(w.columns.min(), w.columns.max() + 1)).interpolate(
        axis=1, limit_area="inside")
    w.columns = [str(c) for c in w.columns]
    return w


# Only lsec_both is read at the off-grid 28-year generation lag (edu_1978);
# the female/primary panels are sampled at grid years only, so they stay as-is.
lsec_both = _interp_panel_to_annual(lsec_both)
...
for country in ajr.index:
    ...
    rows.append({
        ...
        # One biological generation (LAG_GENERATION=28) after the 1950 cohort,
        # read off the annual-interpolated lsec_both panel. Drives T1.2 (β_g)
        # and the M1 generational first-difference.
        "edu_1978": edu_at(lsec_both, country, 1950 + LAG_GENERATION),
        ...
        # Demographic phenotype one generation (LAG_GENERATION=28) after the
        # 1950 and 1978 education cohorts — the endpoints of the M1 first-diff.
        "le_1978": get_wb(le, country, 1950 + LAG_GENERATION),
        "le_2006": get_wb(le, country, 1978 + LAG_GENERATION),
        "le_2020": get_wb(le, country, 2020),
        "tfr_1960": get_wb(tfr, country, 1960),
        "tfr_1975": get_wb(tfr, country, 1975),
        "tfr_1978": get_wb(tfr, country, 1950 + LAG_GENERATION),
        "tfr_2006": get_wb(tfr, country, 1978 + LAG_GENERATION),
...
# The floor is CONTEMPORANEOUS with crossing, not 28 years prior — so we test
...
# M1 first-difference (within-country): one generation of education growth
# (edu_1950 → edu_1978, LAG_GENERATION=28 apart) against the change in the
# demographic phenotype one generation later (outcome_1978 → outcome_2006,
# each read at the script's T+28 cross-generation lag). Differencing sweeps
# out the country fixed effect. The whole step rides on LAG_GENERATION=28 —
# edu_1978 resolves off the 5-year WCDE grid via the annual interpolation
# applied to lsec_both at load.
print(f"\n  M1 first-difference: Δedu_1950→1978 → Δoutcome_1978→2006 "
      f"(one generation, LAG_GENERATION={LAG_GENERATION})")
fd_results = {}
for outcome_late, outcome_early in [("le_2006", "le_1978"),
                                    ("tfr_2006", "tfr_1978")]:
    sub = df.dropna(subset=["edu_1950", "edu_1978",
                            outcome_late, outcome_early]).copy()
    if len(sub) < 10:
        print(f"    {outcome_late}: insufficient data (n={len(sub)})")
        continue
    sub["d_edu"] = sub["edu_1978"] - sub["edu_1950"]
    sub["d_out"] = sub[outcome_late] - sub[outcome_early]
    X = sm.add_constant(sub["d_edu"].values)
    fit = sm.OLS(sub["d_out"].values, X).fit()
    name = outcome_late.split("_")[0]
    print(f"    Δ{name}: n={len(sub)}, "
          f"β(Δedu)={fit.params[1]:+.4f}, t={fit.tvalues[1]:+.2f}, "
          f"p={fit.pvalues[1]:.4f}, R²={fit.rsquared:.3f}")
    fd_results[outcome_late] = {
...
print(f"  child_edu(T+28) ~ parent_edu(T)  per bucket")
print(f"{'═' * 78}")

# For each country, parent_edu = edu_1950, child_edu = edu_1978 — one
# biological generation later (LAG_GENERATION=28). Pool by edu_1950 quartiles.
beta_g = df.dropna(subset=["edu_1950", "edu_1978"]).copy()
...
for bucket in beta_g["bucket"].cat.categories:
    ...
    fit = sm.OLS(sub["edu_1978"].values, X).fit()
    beta = fit.params[1]
    t = fit.tvalues[1]
