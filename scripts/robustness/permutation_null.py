...
PAPER ANCHOR — Chapter 9 §What rules out chance
  Question: could common trends, panel autocorrelation, or serial
            correlation generate the headline β=0.483 by chance?
  Produces: paper claim that 200-iteration permutation null places the
            real β (0.483) at least 53 SDs above either null (within-year shuffle,
            which preserves global temporal trends, and full shuffle, which
            breaks every systematic link).
  Checkin:  checkin/permutation_null.json
...
from _shared import (load_education, write_checkin, LAG_GENERATION,
...
def main():
    ...
    # The parent lag (28) is off the 5-year WCDE grid, so parent_year = year - 28
    # would not match any grid row. Interpolate education to annual for the parent
    # lookup; child years stay on the original 5-year grid.
    _wide = edu.pivot_table(index="country", columns="year",
                            values="lower_sec", aggfunc="first")
    _annual = (interpolate_wide_to_annual(_wide)
               .reset_index()
               .melt(id_vars="country", var_name="year", value_name="lower_sec")
               .dropna(subset=["lower_sec"]))
    _annual["year"] = _annual["year"].astype(int)

    df = edu[["country", "year", "lower_sec"]].copy()
    df["parent_year"] = df["year"] - LAG
    parent = _annual.rename(
