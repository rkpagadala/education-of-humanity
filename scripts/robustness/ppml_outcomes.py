...
#   Predictor: parent 20-24 cohort completion at T. Outcomes: TFR at
#   T+5 (biological parent-of-newborn timing) and U5MR at T+12
#   (childrearing window, multiplicative form). Use: estimator robustness —
#   Poisson PML for right-skewed, non-negative outcomes.
#
# Produces:
...
#     - Total fertility rate at T+LAG_TFR=5
#     - Under-5 mortality at T+LAG_CHILDREARING=12  (per 1,000 live births)
#
#   Specification (multiplicative form):
#     E[y_{i,t+lag} | x] = exp(α_i + β_edu · parent_edu_t + β_gdp · log_gdp_t)
#
...
# newborn timing); U5MR uses LAG_CHILDREARING=12 (childrearing window).
OUTCOME_LAG = {
    "tfr":  LAG_TFR,
...
def main():
    ...
    tfr = ppml(tfr_panel, f"TFR(T+{OUTCOME_LAG['tfr']})")
    u5mr = ppml(u5mr_panel, f"U5MR(T+{OUTCOME_LAG['u5mr']})")
    ...
    write_checkin("ppml_outcomes.json", {
        "notes": (
            ...
            "Outcome-specific lags: TFR at T+LAG_TFR=5 (biological parent-"
            "of-newborn timing); U5MR at T+LAG_CHILDREARING=12 (childrearing "
            "window). Sample: 1975-1990 base years; WCDE v3 lower "
            "secondary completion + WB WDI."
        ),
