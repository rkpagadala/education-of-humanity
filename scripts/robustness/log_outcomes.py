...
#   Predictor: parent 20-24 cohort completion at T. Outcomes: LE at
#   T+28 (cross-generation / time-to-agency); U5MR at T+12 (childrearing
#   window); TFR at T+5 (biological parent-of-newborn timing). Use:
#   functional-form robustness for the headline forward-prediction regression.
#
# Produces:
#   Log-linear robustness for the Table 2 forward-prediction specification.
#   For LE, TFR, and U5MR, runs the headline regression
#
#     outcome_{t+lag} = β · parent_edu_t + γ · log_gdp_t + α_i + u_t
#
#   at the outcome's canonical lag (per _shared.py): LE at
#   T+LAG_GENERATION=28; U5MR at T+LAG_CHILDREARING=12; TFR at
#   T+LAG_TFR=5. Outcome in logs
#   (elasticity/semi-elasticity form) alongside the levels comparator.
#   Education remains in percentage points (0-100), so β on parent is a
#   semi-elasticity: "% change in outcome per pp rise in parent completion."
#
...
from _shared import (  # noqa: E402
    ...
    get_wb_val, LAG_GENERATION, LAG_CHILDREARING, LAG_TFR,
)
...
def build_panel(edu_annual, outcome_df, gdp_df, outcome_name):
    ...
    for c in sorted(edu_annual.keys()):
        ...
        for t in T_YEARS:
            ...
            outcome = get_wb_val(outcome_df, c, t + lag)
...
def main():
    ...
    write_checkin("log_outcomes.json", {
        "notes": (
            ...
            "outcome_{t+lag} on parent_edu_t + log_gdp_t + country FE, "
            "country-clustered SEs. Outcome-specific lags: LE at "
            "T+LAG_GENERATION=28 (time-to-agency); U5MR at "
            "T+LAG_CHILDREARING=12 (childrearing window); TFR at T+LAG_TFR=5 "
            "(biological parent-of-newborn timing). β_log interpreted as "
            "semi-elasticity: percent change in outcome per 1 pp rise in "
            "parental completion. Education remains in percentage points "
            "(0-100). Sample: 1975-1990 base years."
        ),
