...
PAPER ANCHOR — Chapter 9 §The Shape of the Response (Figure~\\ref{fig:cs-event})
  Question: does the parent-to-child education effect compound across
            generations after a country crosses 10% lower-sec completion,
            under a staggered-adoption estimator that survives the TWFE
            pathology?
  Produces: Figure~\\ref{fig:cs-event} — Callaway & Sant'Anna (2021)
            heterogeneity-robust ATT. Headline: +1.3 pp immediately,
            +6.6 at 10 years, +14.9 at 25 years, +21.4 at 35 years; pre-
            treatment coefficients indistinguishable from zero.
  Checkin:  checkin/callaway_santanna.json

Life-history label (§the-generational-lag):
  Predictor: country crosses the 10% lower-sec completion threshold
  at year T (treatment). Outcome: child education stock at T+5..T+35
  (event-time). Generational relation: parent → child compounding
  across one+ generations. Use: identification — staggered-adoption
  event study confirming the compounding-generations signature
  (Prediction 2). Shows +1.3 pp immediately, +21.4 pp at 35 years.
...
Outcomes:
  - Primary: child education at T+28 (matches Table A1)
...
from _shared import (
    PROC, DATA, REGIONS, write_checkin, load_wb, get_wb_val, NAME_MAP,
    LAG_GENERATION,
    interpolate_wide_to_annual,
    completion_at_year,
    FOR_WEB, PAPER_FIGS, WEB_FIGS,
)

# ── Constants ────────────────────────────────────────────────────────
THRESHOLD = 10
PERIODS = list(range(1950, 1995, 5))
# Primary outcome: child education at T+LAG_GENERATION (edu→edu, one-step).
# Secondary outcome: life expectancy at T+LAG_GENERATION (time-to-agency).
# Both share LAG_GENERATION=28 — no TFR in this script.
LAG = LAG_GENERATION
N_BOOT = 500
RNG_SEED = 42
REPO_ROOT = os.path.dirname(os.path.dirname(SCRIPT_DIR))
FIG_DIR = PAPER_FIGS if not FOR_WEB else WEB_FIGS
...
# LAG (28) is off the 5-year WCDE grid; interpolate edu to annual so the
# child-education outcome at t + LAG resolves (canonical-script pattern).
edu_int = edu_wide.copy()
edu_int.columns = [int(c) for c in edu_int.columns]
edu_annual = interpolate_wide_to_annual(edu_int)

treat_cols = [str(y) for y in PERIODS]
complete = edu_wide.dropna(subset=treat_cols)
...
for c in countries:
    for t in PERIODS:
        ...
        child_edu = completion_at_year(edu_annual, c, t + LAG)
        if child_edu is None or np.isnan(child_edu):
            continue
...
def aggregate_results(att_list, weighted=True):
    """Aggregate ATT results: overall and by event time.

    Per CS (2021) Eq 4.2, the overall ATT and per-event-time ATT are weighted
    averages over (g, t) cells, with weights given by the cohort size (number
    of treated countries in cohort g, recorded as n_treat per cell). The
    unweighted average over (g, t) cells is also available for diagnostic
    comparison via weighted=False.
    """
    if not att_list:
        return np.nan, {}
    ...
    if weighted:
        att_agg = (_wmean(post["att"].values, post["n_treat"].values)
                   if len(post) > 0 else np.nan)
    else:
    ...
    for e, grp in df.groupby("event_time"):
        if weighted:
            att_e = _wmean(grp["att"].values, grp["n_treat"].values)
        else:
            att_e = grp["att"].mean()
        dynamic[int(e)] = {"att": att_e, "n_cells": int(len(grp)),
                           "n_treat_total": int(grp["n_treat"].sum())}
...
def bootstrap_cs(outcome_arr, cohort_vec, n_boot=N_BOOT, seed=RNG_SEED):
    ...
    Point estimates and bootstrap CIs are reported under the weighted
    aggregation (CS 2021 Eq 4.2, weights = cohort size n_treat per cell).
    An unweighted variant is also computed and returned for diagnostic
    comparison; the unweighted scheme over-weights small cohorts and is
    NOT the canonical CS estimator.
    """
    rng = np.random.RandomState(seed)
    n_countries = len(cohort_vec)

    # Point estimates — weighted (canonical CS) and unweighted (diagnostic).
    att_list = compute_all_att_gt_fast(outcome_arr, cohort_vec)
    att_agg_w, att_dyn_w = aggregate_results(att_list, weighted=True)
    att_agg_u, att_dyn_u = aggregate_results(att_list, weighted=False)

    # Bootstrap (canonical CS weighted aggregation drives the CIs).
    boot_aggs = []
    boot_dyns = {e: [] for e in att_dyn_w}
    ...
    for b in range(n_boot):
        ...
        boot_att_agg, boot_att_dyn = aggregate_results(boot_att, weighted=True)
    ...
    dyn_ci = {}
    for e in sorted(att_dyn_w):
        vals = boot_dyns.get(e, [])
        vals = [v for v in vals if not np.isnan(v)]
        dyn_ci[e] = {
            "att": att_dyn_w[e]["att"],
            "n_cells": att_dyn_w[e]["n_cells"],
            "n_treat_total": att_dyn_w[e].get("n_treat_total", 0),
    ...
    return {
        ...
        "att_agg": att_agg_w,                  # weighted (canonical CS)
        "att_agg_unweighted": att_agg_u,       # diagnostic only
        "att_dynamic_unweighted": {int(k): v["att"] for k, v in att_dyn_u.items()},
        "att_se": se,
        "att_ci": (ci_lo, ci_hi),
        "att_dynamic": dyn_ci,
    }
...
print(f"\n{'=' * 70}")
print("CALLAWAY-SANT'ANNA ESTIMATOR: Child Education (T+28)")
print(f"{'=' * 70}")
...
for ax, result, title, ylabel in [
    (axes[0], result_edu, "Child Education (T+28)", "ATT (percentage points)"),
...
fig.tight_layout()
for ext in ("pdf", "png"):
    path = os.path.join(FIG_DIR, f"fig_callaway_santanna.{ext}")
    fig.savefig(path, dpi=200, bbox_inches="tight")
print(f"\n  Figure saved to paper/figures/fig_callaway_santanna.{{pdf,png}}")
plt.close(fig)
...
write_checkin("callaway_santanna.json", {
    "method": (
        "Callaway & Sant'Anna (2021) group-time ATT, aggregated per Eq 4.2 "
        f"with weights = cohort size (n_treat per cell). "
        f"Treatment: lower secondary ≥ {THRESHOLD}%. "
        f"Control: not-yet-treated at each (g,t); always-treated countries "
        f"excluded from controls per the standard CS handling. "
        f"Panel: {N} countries, {PERIODS[0]}–{PERIODS[-1]}, 5yr. "
        f"Outcomes (child education, life expectancy) both use "
        f"LAG_GENERATION={LAG_GENERATION}y. "
        f"Bootstrap: {N_BOOT} reps, clustered by country."
    ),
    "sample_composition": {
        "n_always_treated": len(always_ids),
        "n_timing_treated": len(timing_ids),
        "n_never_treated": len(never_ids),
        "cohort_sizes": {
            str(g): int(sum(1 for _, gg in cohort_map.items() if gg == g))
            for g in timing_vals
        },
    },
    "child_education": {
        "att_aggregate": round(result_edu["att_agg"], 2),
        "att_aggregate_unweighted": round(result_edu["att_agg_unweighted"], 2),
        ...
        "event_study_unweighted": {
            str(e): round(v, 2)
            for e, v in result_edu["att_dynamic_unweighted"].items()
        },
    },
    "life_expectancy": {
        "att_aggregate": round(result_le["att_agg"], 2),
        "att_aggregate_unweighted": round(result_le["att_agg_unweighted"], 2),
        ...
        "event_study_unweighted": {
            str(e): round(v, 2)
            for e, v in result_le["att_dynamic_unweighted"].items()
        },
    },
