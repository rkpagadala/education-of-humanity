"""
Reproduce Table A1 from "The Long Childhood."
...
from _shared import (PROC, DATA, CHECKIN, REGIONS as NON_SOVEREIGN,
                     write_checkin, add_canonical_aliases, LAG_GENERATION,
...
# to annual so lagged lookups resolve (on-grid at the old 25). Keep string columns
# so the str(year) lookups below are unchanged.
agg.columns = agg.columns.astype(int)
agg = interpolate_wide_to_annual(agg)
...
# Expansion-sample filter: child-year lower-sec completion in [10, 90]
panel = panel[(panel["child"] >= 10) & (panel["child"] <= 90)].reset_index(drop=True)
print(f"Expansion panel: {len(panel)} obs, {panel['country'].nunique()} countries")
