...
#   (child_t ~ parent_{t-28}, country FE, clustered SE).
#
...
from _shared import PROC, REGIONS, write_checkin, LAG_GENERATION  # noqa: E402
...
def main():
    ...
    # Build all horizons off the annual-interpolated grid: the test varies the
    # CHILD-cohort step (5 / 10 / 1 years), but the parent lag (28) is off the
    # 5-year grid, so the parent at child_year - 28 must be interpolated in every
    # horizon. Child cohort years are grid points, so their values are unchanged.
    p5  = build(wa, cohorts_5)
    p10 = build(wa, cohorts_10)
    ...
    write_checkin("period_length.json", {
        "notes": (
            ...
            "parent_{t-28}, country FE, country-clustered SEs. "
