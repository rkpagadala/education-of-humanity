...
Life-history label (§the-generational-lag):
  Predictor: WCDE 20-24 cohort completion at year T (the cohort that
  finished schooling 5-9 years before year T). At T=1960-1990 spans
  the post-war expansion period.
  Outcome: outcome at T + outcome-specific lag.
    - log GDP, LE, U5MR, and Panel B (lower-sec at T+lag) use
      LAG_GENERATION=28 — cross-generation step / time-to-agency.
    - TFR uses LAG_TFR=5 — biological parent-of-newborn timing.
      Cohort age 20-24 at T is at the reproductive peak by T+5.
  Use: identification (forward-prediction discipline rules out
  reverse causality), not the lag with strongest within-cohort fit.
  The contemporaneous (lag-0) and childrearing-window (lag-15) lags
  are in table_contemporaneous.py and table_short_15yr.py.

Panel A — outcomes at T+lag on lower-secondary (or primary, for TFR) at T:
  ...
  (4) col 3 + year FE
  Lag is outcome-specific: TFR uses LAG_TFR=5; log GDP, LE, U5MR use
  LAG_GENERATION=28.

Panel B — lower-secondary at T+28 on log GDP per capita at T:
...
# This stepwise table is the generational-vantage view: GDP, LE, U5MR, and child
# education (lower-sec) all enter at LAG_GENERATION=28 (one generation later);
# TFR uses LAG_TFR=5 (biological parent timing). Distinct from the canonical
# per-outcome-lag table (table_contemporaneous.py: GDP@0, LE@12, TFR@5, U5MR@12).
OUTCOME_LAG = {
    "log_gdp_tp":  LAG_GENERATION,
    "log_le_tp":   LAG_GENERATION,
    "log_tfr_tp":  LAG_TFR,         # column name kept for compatibility; underlying lag is LAG_TFR=5
    "log_u5_tp":   LAG_GENERATION,
    "low_tp":      LAG_GENERATION,  # parent → child lower-sec (Panel B outcome)
}
# Aliases for the raw (un-logged) columns we pull from WB.
_RAW_OUTCOME_LAG = {
    "gdp_tp":  LAG_GENERATION,
    "le_tp":   LAG_GENERATION,
    "tfr_tp":  LAG_TFR,
    "u5_tp":   LAG_GENERATION,
}
...
for c in sorted(edu_long["country"].unique()):
    ...
    for t in EDU_YEARS:
        ...
        gdp_tp = _val(gdp_df, c, t + _RAW_OUTCOME_LAG["gdp_tp"])
        le_t   = _val(le_df,  c, t)
        le_tp  = _val(le_df,  c, t + _RAW_OUTCOME_LAG["le_tp"])
        tfr_t  = _val(tfr_df, c, t)
        tfr_tp = _val(tfr_df, c, t + _RAW_OUTCOME_LAG["tfr_tp"])  # LAG_TFR=5
        u5_t   = _val(u5_df,  c, t)
        u5_tp  = _val(u5_df,  c, t + _RAW_OUTCOME_LAG["u5_tp"])
        rows.append({
            "country": c, "t": t,
...
# ── Panel B specs ────────────────────────────────────────────────────────────
# Outcome: lower-secondary at T+28; predictor: log GDP at T (and initial edu).
# We need lower-sec at T+28 on the panel — the existing producer pulls this
# from low_tp28 which we don't have here. Let's add it.
print(f"\nAdding lower-sec at T+{OUTCOME_LAG['low_tp']} column "
      "(Panel B outcome, LAG_GENERATION), linearly interpolated off the "
      "5-year WCDE grid so non-grid lags (e.g. T+28) are supported...")
# Per-country (year -> lower_sec) interpolation points (NaNs dropped). The
# generational lag (28) is not a multiple of the 5-year WCDE grid, so the
# outcome year T+28 falls between grid points and must be interpolated; an
# exact-year lookup would silently drop every Panel B observation.
_edu_pts = {}
for c, g in edu_long.dropna(subset=["lower_sec"]).groupby("country"):
    gs = g.sort_values("year")
    _edu_pts[c] = (gs["year"].to_numpy(dtype=float),
                   gs["lower_sec"].to_numpy(dtype=float))
extra_rows = []
for _, row in panel.iterrows():
    tp = row["t"] + OUTCOME_LAG["low_tp"]
    pts = _edu_pts.get(row["country"])
    if pts is None or tp < pts[0][0] or tp > pts[0][-1]:
        extra_rows.append(np.nan)   # outside the observed range → no extrapolation
    else:
        extra_rows.append(float(np.interp(tp, pts[0], pts[1])))
...
write_checkin(
    "table7_stepwise.json",
    {
        "method": (
            ...
            "at T+lag on lower-secondary completion at T (primary for "
            "log(TFR)) with increasing controls: c1 edu only, c2 + initial "
            "outcome, c3 + log GDP, c4 + year FE. Panel B regresses "
            "lower-secondary at T+28 on log GDP at T: b1 GDP only, b2 + "
            "initial edu, b3 GDP only + year FE, b4 + initial edu + year FE. "
            "Country fixed effects throughout; country-clustered standard "
            "errors. Outcome-specific lag: TFR uses LAG_TFR="
            f"{LAG_TFR} (biological parent timing); log GDP, LE, U5MR, and "
            f"Panel B (lower-sec) use LAG_GENERATION={LAG_GENERATION} "
            "(cross-generation / time-to-agency). Two sample versions: max "
            "(drop NAs only on regressors used) and common (drop NAs on the "
            "union of vars across the most demanding spec; same sample as "
            "the GDP row of the previous Table 7 = 927/179)."
        ),
