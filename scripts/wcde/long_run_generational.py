...
Key question: does the T-28 parental multiplier hold over a 100-year horizon?
...
from _shared import REPO_ROOT, write_checkin, LAG_GENERATION
...
def v(df_w, country, year):
    # Linear-interpolate to annual over the 5-year WCDE cohort grid: the
    # one-generation lag (LAG_GENERATION=28) is off the 5-year grid.
    if country not in df_w.index:
        return np.nan
    row = df_w.loc[country].dropna()
    if len(row) == 0:
        return np.nan
...
# ── Long-run panel with T-28 lag (one generation, MAC≈28.7) ───────────────────
...
h("---")
h()
h("## Table 3 — The Generational Chain (T-28 Parent-Child Pairs for Key Countries)")
h()
for c in KEY[:6]:
    h(f"**{c}:**")
    h()
