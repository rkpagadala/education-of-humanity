...
Does education at time T predict GDP, life expectancy, TFR, and U5MR,
each read at its own biological lag?

Life-history label (§the-generational-lag):
  Predictor: WCDE 20-24 cohort completion at year T.
  Outcomes are read at per-outcome biological lags, not a single T+28:
    - TFR    at T+5   (LAG_TFR — schooled cohort at reproductive peak)
    - LE     at T+12  (LAG_LE / childrearing window — LE at birth is
                       mortality-dominated, mostly U5MR)
    - U5MR   at T+12  (LAG_CHILDREARING — the cohort's own children in
                       the under-5 window)
    - log GDP at T+28 (kept at the forward horizon here: contemporaneous
                       GDP would be degenerate against its own initial
                       condition in the edu-vs-initial-income horse race)
  The edu → edu step (reverse / parental-transmission test) is also the
  cross-generation step at T+28 (~28 years; biological generation
  cycle MAC ≈ 28). The "T+28" headline elsewhere is generational time,
  not the per-outcome lag. For the older contemporaneous (lag 0) and
  childrearing-window specs see table_contemporaneous.py and
  table_short_15yr.py.
...
Design:
  For each country-year T, regress each outcome (at its own lag) on:
  ...
  Key comparison: which explains more of each outcome —
...
Education lag (edu → edu): 28 years (one generation — the T-28 cohort
  mechanism). Also tested: 10-year and 15-year lags (human capital
  entering workforce sooner).

Education measure: lower secondary completion (policy-critical level)
Also tested: primary, upper secondary

Outcomes (each at its biological lag):
  1. log GDP per capita (T+28)  — education drives income
  2. Life expectancy e0 (T+12)  — education drives health
  3. TFR (T+5)                  — education drives fertility transition
  4. U5MR (T+12)                — education drives child survival
...
from _shared import LAG_LE, LAG_TFR, LAG_CHILDREARING, LAG_GENERATION
...
# Annual WB life expectancy (SP.DYN.LE00.IN) — the paper's canonical LE source.
# Needed at annual resolution because LE now routes to the childrearing window
# (T+12), which the 5-year WCDE e0.csv cannot serve. Mirrors the U5MR source.
le_raw = load_wb("life_expectancy_years.csv")
print(f"  LE: {len(le_raw)} countries, years {le_raw.columns[0]}–{le_raw.columns[-1]}")
...
def get_e0(country_wcde, year):
    # Annual WB life expectancy, name-mapped like get_u5mr (WCDE → WB-short).
    key = _SHARED_NAME_MAP.get(country_wcde, country_wcde).lower()
    for k in [country_wcde.lower(), key]:
        if k in le_raw.index:
            try:
                v = float(le_raw.loc[k, str(year)])
                return v if not np.isnan(v) and v > 0 else np.nan
            except (KeyError, ValueError, TypeError): pass
    return np.nan
...
# ── Build panel ───────────────────────────────────────────────────────────────
# T years where T+28 is within our data range
EDU_YEARS = [1960, 1965, 1970, 1975, 1980, 1985, 1990]  # T+28 = 1988..2018
...
for c in countries:
    ...
    for t in EDU_YEARS:
        ...
        tp28 = t + LAG_GENERATION   # one-generation forward horizon (28 yrs)
        tp15 = t + 15
        tp10 = t + 10
        ...
        gdp_t    = get_gdp(c, t)
        gdp_tp28 = get_gdp(c, tp28)
        gdp_tp15 = get_gdp(c, tp15)
        gdp_tp10 = get_gdp(c, tp10)

        # Per-outcome canonical lags (scripts/_shared.py): LE and U5MR route
        # to the childrearing window, TFR to the parental reproductive peak.
        # Each column's suffix now names its own per-outcome lag: the value
        # held is the outcome at T + its canonical lag (GDP T+28, LE T+12,
        # TFR T+5, U5MR T+12). (GDP stays at the forward T+28 horizon —
        # contemporaneous GDP would be degenerate against its own initial
        # condition in the edu-vs-initial horse race.)
        e0_t    = get_e0(c, t)
        e0_tp12 = get_e0(c, t + LAG_LE)            # LE at childrearing window (T+12)

        tfr_t    = get_tfr(c, t)
        tfr_tp5 = get_tfr(c, t + LAG_TFR)          # TFR at reproductive peak (T+5)

        u5mr_t    = get_u5mr(c, t)
        u5mr_tp12 = get_u5mr(c, t + LAG_CHILDREARING)   # U5MR at childrearing window (T+12)

        if any(np.isnan(x) for x in [low, pri]): continue

        # Education at T+LAG_GENERATION (reverse regression: GDP(T) -> edu(T+28)).
        # 28 is off the 5-year WCDE grid, so interpolate to annual.
        low_tp28_val = np.nan
        _ls = edu_c["lower_sec"].dropna().sort_index()
        if len(_ls) and _ls.index.min() <= tp28 <= _ls.index.max():
            v_tp28 = float(np.interp(tp28, _ls.index.astype(float), _ls.values.astype(float)))
            low_tp28_val = float(np.clip(v_tp28, 0, 100))

        rows.append({
            "country": c, "t": t, "tp28": tp28,
            "low_t": low, "pri_t": pri, "upp_t": upp, "col_t": col_v,
            "low_tp28": low_tp28_val,
            "log_gdp_t":    np.log(gdp_t)    if not np.isnan(gdp_t)    else np.nan,
            "log_gdp_tp28": np.log(gdp_tp28) if not np.isnan(gdp_tp28) else np.nan,
            ...
            "gdp_growth_28": (np.log(gdp_tp28) - np.log(gdp_t)) if not np.isnan(gdp_t) and not np.isnan(gdp_tp28) else np.nan,
            "e0_t": e0_t, "e0_tp12": e0_tp12,
            "tfr_t": tfr_t, "tfr_tp5": tfr_tp5,
            "u5mr_t": u5mr_t, "u5mr_tp12": u5mr_tp12,
        })
...
for col in ["e0_t", "e0_tp12", "tfr_t", "tfr_tp5", "u5mr_t", "u5mr_tp12"]:
    panel[f"log_{col}"] = np.log(panel[col].where(panel[col] > 0))

print(f"  Panel: {len(panel)} obs, {panel['country'].nunique()} countries")
print(f"  GDP coverage (T+28): {panel['log_gdp_tp28'].notna().sum()} obs")
print(f"  E0 coverage (T+12): {panel['e0_tp12'].notna().sum()} obs")
...
# GDP outcomes
for outcome, label in [("log_gdp_tp28","log GDP(T+28)"),("gdp_growth_28","GDP growth T→T+28")]:
    results[outcome] = {}
...
for spec, xcols, fe in [
    ...
    coefs, r2, n = run_ols(xcols, "e0_tp12", panel, fe=fe)
    results["e0"][spec] = (coefs, r2, n)
    coef_str = ", ".join(f"{k}:{v:.3f}" for k,v in coefs.items()) if coefs else "n/a"
    print(f"  e0(T+28) | {spec}: {coef_str}, R²={r2:.3f}, n={n}")
...
for spec, xcols, fe in [
    ...
    coef_str = ", ".join(f"{k}:{v:.3f}" for k,v in coefs.items()) if coefs else "n/a"
    print(f"  TFR(T+28) | {spec}: {coef_str}, R²={r2:.3f}, n={n}")
...
for spec, xcols in [
    ...
    print(f"  TFR(T+28) | {spec}: {coef_str}, R²={res['r2']:.3f}, n={res['n']}, "
...
for spec, xcols in [
    ...
    stars = "***" if p < 0.01 else "**" if p < 0.05 else "*" if p < 0.10 else ""
    coef_str = ", ".join(f"{k}:{v:.4f}" for k, v in res["coefs"].items())
    print(f"  log(TFR)(T+28) | {spec}: {coef_str}, R²={res['r2']:.3f}, n={res['n']}, "
          f"countries={res['countries']}, pri p={p:.4g}{stars}")

for outcome_label, log_x, log_y in [
    ("log(LE)",   "log_e0_t",   "log_e0_tp12"),
    ("log(TFR)",  "log_tfr_t",  "log_tfr_tp5"),
    ("log(U5MR)", "log_u5mr_t", "log_u5mr_tp12"),
]:
    key = log_y
    results[key] = {}
    for spec, xcols in [
        ...
        if res is None:
            print(f"  {outcome_label}(T+28) | {spec}: insufficient data")
            continue
        b = res["coefs"]["low_t"]
        p = res["pvals"]["low_t"]
        stars = "***" if p < 0.01 else "**" if p < 0.05 else "*" if p < 0.10 else ""
        coef_str = ", ".join(f"{k}:{v:.4f}" for k, v in res["coefs"].items())
        print(f"  {outcome_label}(T+28) | {spec}: {coef_str}, R²={res['r2']:.3f}, "
...
for level in ["pri_t","low_t","upp_t","col_t"]:
    coefs, r2, n = run_ols([level,"log_gdp_t"], "log_gdp_tp28", panel, fe=True)
    edu_level_r2[level] = (coefs, r2, n)
    coef_str = ", ".join(f"{k}:{v:.3f}" for k,v in coefs.items()) if coefs else "n/a"
    print(f"  FE log GDP(T+28) ~ {level} + log_gdp_t: {coef_str}, R²={r2:.3f}, n={n}")

# ── Lag comparison (lower sec → GDP) ─────────────────────────────────────────
print("\nLag sensitivity (OLS, lower_sec → log GDP):")
lag_results = {}
for lag, out_col in [(10,"log_gdp_tp10"),(15,"log_gdp_tp15"),(28,"log_gdp_tp28")]:
    coefs, r2, n = run_ols(["low_t","log_gdp_t"], out_col, panel)
    lag_results[lag] = (coefs, r2, n)
    coef_str = ", ".join(f"{k}:{v:.3f}" for k,v in coefs.items()) if coefs else "n/a"
    print(f"  T+{lag}: {coef_str}, R²={r2:.3f}, n={n}")

# ── Reverse regression: GDP(T) -> edu(T+28) [Panel B asymmetry test] ─────────
print("\nReverse regression (Panel B): GDP(T) -> edu(T+28)...")
results_rev = {}
for spec, xcols, fe in [
    ("OLS: GDP only",       ["log_gdp_t"],            False),
...
h()
h("*Direct test: does education at time T predict GDP, life expectancy, TFR, and U5MR, "
  "each read at its own biological lag (GDP at T+28; TFR at T+5; LE and U5MR at T+12)?*")
h()
h("## Setup")
h()
h(f"- **Countries:** {panel['country'].nunique()} (WCDE v3, both sexes, 20–24 cohort)")
h(f"- **T years:** 1960, 1965, 1970, 1975, 1980, 1985, 1990 (GDP outcome at T+28 = 1988–2018; "
  f"LE/U5MR at T+12, TFR at T+5)")
...
h(f"- **Panel:** {panel['log_gdp_tp28'].notna().sum()} obs with GDP; "
  f"{panel['e0_tp12'].notna().sum()} obs with E0; "
  f"{panel['tfr_tp5'].notna().sum()} obs with TFR")
h()
h("**Key comparison in each model:** does initial education or initial outcome level")
h("better explain each outcome at its own biological lag (GDP at T+28; TFR at T+5; "
  "LE and U5MR at T+12)?")
...
# ── GDP table ──────────────────────────────────────────────────────────────────
h("## 1. Education → Income (log GDP per capita in 28 years)")
h()

c_gdp = results["log_gdp_tp28"]
pipe_table(
...
# GDP growth table
c_gr = results["gdp_growth_28"]
h("**Growth version: GDP growth rate T→T+28 (OLS and FE)**")
h()
pipe_table(
    ["Model","Edu β (low_sec)","GDP β (initial)","R²","N"],
    [
...
# ── E0 table ───────────────────────────────────────────────────────────────────
h("## 2. Education → Life Expectancy (e0 in 28 years)")
h()
c_e0 = results["e0"]
pipe_table(
...
# ── TFR table ──────────────────────────────────────────────────────────────────
h("## 3. Education → Fertility (TFR in 28 years)")
h()
c_tfr = results["tfr"]
pipe_table(
...
# Extract key numbers
edu_gdp_ols_joint   = results["log_gdp_tp28"]["OLS: edu + GDP"][0]
edu_gdp_fe_joint    = results["log_gdp_tp28"]["FE:  edu + GDP"][0]
edu_gdp_ols_r2_edu  = results["log_gdp_tp28"]["OLS: edu only"][1]
edu_gdp_ols_r2_gdp  = results["log_gdp_tp28"]["OLS: GDP only"][1]
edu_gdp_ols_r2_both = results["log_gdp_tp28"]["OLS: edu + GDP"][1]
...
if edu_gdp_fe_joint:
    ...
    h(f"secondary completion at T predicts a **{edu_b:+.4f} log-point increase in GDP at T+28**")
...
if edu_e0_fe:
    ...
    h(f"**FE:** A 1 pp rise in lower secondary completion at T predicts a"
      f" **{edu_b_e0:+.3f} year increase in life expectancy at T+12**,")
    ...
    h("The T+12 childrearing-window lag captures the mother→child transmission: women")
    h("educated at T bear children whose under-five survival dominates the life expectancy")
    h("measure (LE at birth is mortality-driven, mostly U5MR).")
h()
...
if edu_tfr_fe:
    ...
    h(f"**FE:** A 1 pp rise in lower secondary completion at T predicts a"
      f" **{edu_b_tfr:+.4f} change in TFR at T+5**,")
    ...
    h("more educated women have fewer children. The T+5 lag captures the reproductive-peak")
    h("timing — women educated at T enter their prime fertility years about five years later.")
h()
...
h("    ↓ T+28")
h("Education(T) → Education(T+28)   [home-niche transmission, β=0.49 FE]")
h("    ↓")
h("Education(T) → GDP(T+28)         [human capital → income]")
h("Education(T) → e0(T+12)          [maternal education → infant survival]")
h("Education(T) → TFR(T+5)          [educated women → fewer children]")
h("```")
h()
h("Each outcome sits at its own biological lag (GDP T+28, LE/U5MR T+12, TFR T+5); these")
h("are the *generational* channels: educated parents raise children who survive, earn more,")
h("live longer, and have fewer (better-educated) children. Educated workers also raise GDP")
h("contemporaneously, so the forward GDP horizon is not the only income channel.")
h()
h("**The causal direction argument** rests on three things:")
h("1. Temporal precedence (education at T cannot be caused by later outcomes, each read")
h("   forward at its own lag)")
...
h("degree. Richer countries invest more in education, which builds more GDP. Our T+28 lag")
...
h("demographic transitions ~28 years later.")
h()
h("---")
h()
h("*Data: WCDE v3 (education, TFR, e0), World Bank (GDP). T years 1960–1990, T+28 = 1988–2018.*")
...
checkin_numbers["T2-fwd-edu-R2"] = _safe_r2(results["log_gdp_tp28"]["FE:  edu only"])

# Forward: edu + GDP -> GDP (FE)
checkin_numbers["T2-GDP-beta"] = _safe_coef(results["log_gdp_tp28"]["FE:  edu + GDP"], "low_t")
checkin_numbers["T2-GDP-init"] = _safe_coef(results["log_gdp_tp28"]["FE:  edu + GDP"], "log_gdp_t")
checkin_numbers["T2-GDP-R2"] = _safe_r2(results["log_gdp_tp28"]["FE:  edu + GDP"])
...
for tag, key, init_x, label in [
    ("LE",   "log_e0_tp12",   "log_e0_t",   "log(LE)"),
    ("TFR",  "log_tfr_tp5",   "log_tfr_t",  "log(TFR)"),
    ("U5MR", "log_u5mr_tp12", "log_u5mr_t", "log(U5MR)"),
]:
...
# Reverse conditional: GDP + init edu -> edu(T+28)
