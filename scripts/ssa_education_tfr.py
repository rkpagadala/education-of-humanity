...
Life-history label (§the-generational-lag):
  Predictor: 20-24 cohort primary completion at T, restricted to SSA.
  Outcome: TFR at T+5 (LAG_TFR, reproductive-peak lag) in the most-stuck
  region of the panel. Tests that the parent→daughter primary chain
  holds even in early-transition settings. Use: subsample robustness.
...
Three cuts:
  ...
  (1) SSA-only FE panel: education(T) -> TFR(T+5), country fixed
...
T = 1960..1990 (5yr step), lag = LAG_TFR (5y) — biological parent
timing.  The parent cohort age 20-24 at T is age 25-29 at T+5
(reproductive peak); period TFR at T+5 reflects that cohort's own
fertility.
...
LAG = LAG_TFR    # biological parent-of-newborn timing (5y)
...
def run_panel():
    print("=" * 78)
    print(f"(1) + (3) SSA-ONLY FE PANEL: education(T) -> TFR(T+{LAG})")
    print("=" * 78)
    ...
    print()
    print("Reference: global primary -> TFR(T+5) R^2 = 0.65")
...
def main():
    ...
    checkin = {
        "method": (
            f"Sub-Saharan Africa restriction (UN M49, 48 countries) of "
            f"the paper's primary -> TFR claim. Three cuts: (1) FE panel, "
            f"edu(T) -> TFR(T+{LAG}), three education levels (primary, "
            f"lower-sec, upper-sec); (2) primary completion at TFR<3.65 "
            f"crossing year for SSA crossers; (3) composition-by-level "
            f"comparison within SSA. T=1960-1990 (5yr step), lag=LAG_TFR={LAG}."
        ),
