...
OUT_PATH = os.path.join(PAPER_FIGS if not FOR_WEB else WEB_FIGS,
                        fig_file("cumulative_developed.png"))
...
def main():
    ...
    ax1.plot(years, pct_developed, color=color_pct, linewidth=2.5,
             label=L("% of world population", "世界人口占比"))
    ax1.set_xlabel(L("Year", "年份"), fontsize=12)
    ax1.set_ylabel(L("Share of world population (%)", "世界人口占比（%）"),
                   fontsize=12, color=color_pct)
    ...
    ax2.plot(years, n_countries, color=color_n, linewidth=1.5,
             linestyle="--", alpha=0.7, label=L("Number of countries", "国家数量"))
    ax2.set_ylabel(L("Number of countries", "国家数量"),
                   fontsize=12, color=color_n)
    ...
    annotations = [
        ...
        (1987, L("S. Korea", "韩国")),
        (1994, L("China", "中国")),
        (2014, L("Bangladesh", "孟加拉国")),
        (2022, L(f"{n_countries[-1]} countries", f"{n_countries[-1]} 个国家")),
    ]
    ...
    if FOR_WEB:
        ax1.set_title(
            L("Cumulative share of world population in 'developed' countries\n"
              f"(TFR < {TFR_THRESHOLD}, LE > {LE_THRESHOLD} — 1960 USA benchmarks)",
              "处于「已发展」国家的世界人口累计占比\n"
              f"（生育率 < {TFR_THRESHOLD}、预期寿命 > {LE_THRESHOLD} —— 1960 年美国基准）"),
