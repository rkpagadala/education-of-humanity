...
Generates the generational-β-vs-baseline figure (fig:beta-baseline) for:
  "The Long Childhood"
...
What it does:
  ...
  (state + home-niche compounding), β→0 near ceiling, β always positive
  (durability).
...
Key parameters:
  WINDOW_SIZE = 28 years (6 child cohorts at 5-year intervals; one generation)
  STEP        = 10 years
  LAG         = 28 years (one generation, MAC≈28.7)
...
OUT          = os.path.join(PAPER_FIGS if not FOR_WEB else WEB_FIGS, "beta_vs_baseline.png")
OUT_SCATTER  = os.path.join(PAPER_FIGS if not FOR_WEB else WEB_FIGS,
                            "beta_vs_baseline_scatter.png")

# ── parameters ────────────────────────────────────────────────────────────────
WINDOW_SIZE = LAG_GENERATION    # years (6 cohorts at 5-year intervals; one generation)
STEP        = 10    # years between window starts
LAG         = LAG_GENERATION    # one generation (off the 5-yr grid → annual interp below)
...
# Parent lookup is child_year - LAG (28); 28 is off the 5-year cohort grid, so
# interpolate to annual resolution before any lagged lookup (was on-grid at LAG=25).
wide = interpolate_wide_to_annual(wide)
...
# window (WINDOW_SIZE=28, STEP=10, LAG=28, MIN_OBS=3). Bin observations by
...
if FOR_WEB:
    ax.set_title(
        "Generational β Varies With Baseline Education Level\n"
        "Sliding window (28 years), within-country OLS, 1900–2015",
        fontsize=12,
    )
...
#
# To keep the regression numerically stable, drop windows where β
# is outside [-3, 5] (a few near-singular cases at extremes); in practice
...
if FOR_WEB:
    ax2.set_title("Universality of ceiling compression: country-window scatter\n"
                  ...
                  f"(WINDOW_SIZE={WINDOW_SIZE}, STEP={STEP}, LAG={LAG}, MIN_OBS={MIN_OBS})",
                  fontsize=11)
...
# Compute specific beta values needed by verify_the_long_childhood.py
