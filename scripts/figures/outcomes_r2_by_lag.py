...
Lag-decay analysis across all four development outcomes the paper cares
about, plotted on a single shared axis: within-country R² vs the lag
between predictor and outcome, for education (solid) and log GDP per
capita (dashed), with each outcome a matched colour pair.
...
What it does:
  ...
  Plots all outcomes' R² curves vs lag on one shared axis (education
  solid, log GDP dashed; matched colour per outcome), with generational
  horizons (25, 50, 75) marked.
...
from _shared import PROC, DATA, REPO_ROOT, write_checkin, FOR_WEB, PAPER_FIGS, WEB_FIGS

WCDE_PROC = PROC
WB_DIR    = DATA
OUT       = os.path.join(PAPER_FIGS if not FOR_WEB else WEB_FIGS, "outcomes_r2_by_lag.png")
