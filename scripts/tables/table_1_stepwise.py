# =============================================================================
# PAPER ANCHOR — Chapter 9 §Education vs GDP at Population Scale (headline table)
#   Question: on the active-expansion subsample (parental completion <30%),
#             how much does adding contemporaneous log GDP move the
#             parental-education coefficient? Does GDP have any independent
#             pull on child completion one generation forward?
#   Produces: Table~\ref{tab:headline}. Headline β_g = 1.376 ($t=16.4$);
#             column 2 puts log GDP in and β barely moves (1.376→1.270),
#             GDP attaches only a weakly significant slope.
#   Checkin:  checkin/table_1_stepwise.json
# =============================================================================
# Specification — four columns of increasing controls on the same common
# sample so the parental-education coefficient is directly comparable:
#
#     (1) child ~ parent,                         C-FE
...
from _shared import (PROC, DATA, REGIONS, write_checkin, LAG_GENERATION,  # noqa: E402
...
def load_panel():
    ...
    # Parent lookups are at child_yr - LAG_GENERATION (28), off the 5-year cohort
    # grid; interpolate to annual so lagged lookups resolve (on-grid at the old 25).
    low_w = interpolate_wide_to_annual(low_w)
