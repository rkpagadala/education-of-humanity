...
NEAR_MISSES = [
    ("Cuba",           1993, "Soviet collapse: GDP fell ~35%, imports cut by roughly three-quarters"),
...
edu_wide = load_education("lower_sec_both.csv")
...
# Standardize to canonical short-form country names so famine and near-miss
# lookups join on the same key as every other script in the audit.
edu["country"] = standardize_country_series(edu["country"])
edu = edu.dropna(subset=["country"])
...
def lookup_education(country, famine_year):
    ...
    canon = standardize_country_name(country)
    if canon is None:
        return country, np.nan
    country_data = edu[edu["country"] == canon]
    if country_data.empty:
        return canon, np.nan
    # Find closest available year
    closest_idx = (country_data["year"] - famine_year).abs().idxmin()
    row = country_data.loc[closest_idx]
    return canon, row["lower_sec_pct"]
...
# ── Ireland 1845-1849: colonial-famine class ─────────────────────────────

print("=" * 90)
print("IRELAND 1845-1849: Same Crown, same press, education suppressed for 150 years")
print("=" * 90)
print()
print("The Great Irish Famine killed ~1,000,000 and drove ~1,500,000 into emigration.")
print("Mortality concentrated in Catholic west/south; near-zero in Protestant east Ulster.")
print("The blight struck the whole island; the famine ran along the literacy gradient.")
print()
print("1841 Census literacy (population aged 5+):")
print()
print("  National aggregate:                       ~47%")
print("  Protestant Ulster (east):                 ~60%")
print("  Catholic Connacht / west Munster:         <30%")
print()
print("Causal structure:")
print("  Penal Laws (1695-1829): restricted Catholic schooling")
print("  National School Act (1831): only 14 years before blight (1845)")
print("  Famine zones reached thinnest by the new system")
print("  150 years of colonial education suppression preceded the blight")
print()
print("Excess mortality:    ~1,000,000 (Mokyr 1983; Ó Gráda 1999)")
print("Emigration outflow:  ~1,500,000 (famine decade)")
print()
print("Key sources:")
print("  Mokyr (1983) Why Ireland Starved — county-level regression: literacy")
print("    negatively predicts excess mortality net of potato dependence")
print("  Ó Gráda (1995, 1999) — confirms gradient; ~1m excess deaths")
print("  Census of Ireland 1841 — literacy by county")
print()
print("Ireland 1845 is the third exception class to the prediction:")
print("  external force, sustained across generations,")
print("  holding an educated population from forming.")
print()
...
plt.tight_layout()
outpath = os.path.join(PAPER_FIGS if not FOR_WEB else WEB_FIGS, "famine_vs_education.png")
