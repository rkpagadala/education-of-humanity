...
hap["country"] = standardize_country_series(hap["country"])
hap = hap.dropna(subset=["country"])

# ── Load education data (long format, then interpolate to annual) ───
edu_raw = load_education("lower_sec_both.csv")
edu_long = edu_raw.melt(id_vars="country", var_name="year", value_name="education")
edu_long["year"] = edu_long["year"].astype(int)
edu_long["country"] = standardize_country_series(edu_long["country"])
edu_long = edu_long.dropna(subset=["country", "education"])

edu_series = interpolate_to_annual(edu_long, "education")
edu_annual = pd.concat(
    [s.rename("education").reset_index().assign(country=c) for c, s in edu_series.items()],
    ignore_index=True,
)

# ── Load GDP data (wide → long via load_wb) ─────────────────────────
gdp_wide = load_wb("gdppercapita_us_inflation_adjusted.csv")
gdp_long = gdp_wide.reset_index().melt(
    id_vars=gdp_wide.index.name or "Country",
    var_name="year",
    value_name="gdp",
)
gdp_long.columns = ["country", "year", "gdp"]
gdp_long["country"] = standardize_country_series(gdp_long["country"])
...
gdp_long = gdp_long.drop_duplicates(subset=["country", "year"])
