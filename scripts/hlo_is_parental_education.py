...
largely a downstream function of their parents' education 28 years
ago — i.e., what the paper's home-niche mechanism predicts.

HLO tests 15-year-olds today. Their parents are ~30 today, and had
their formative education ~28 years ago (when the parents were
~20-24). So:

  HLO_today(15yo) = f( parental_education_28yrs_ago(20-24) )
...
Test:
  ...
  4. Then re-run the TFR / U5MR / LE horse race, but decompose HLO
     ...
     residual ("pure school quality" independent of the home niche).
...
from _shared import DATA, PROC, REPO_ROOT, load_wide_indicator, REGIONS, write_checkin, FOR_WEB, PAPER_FIGS, WEB_FIGS
...
def main():
    ...
    print(f"  (parents' 20-24 education 28 years ago → "
    ...
    # ── Decompose HLO into home-niche-predicted + residual ──────────────
    d = df.dropna(subset=["hlo_sec", "lsec_1990"]).copy()
    X = sm.add_constant(d["lsec_1990"].astype(float))
    m = sm.OLS(d["hlo_sec"].astype(float), X).fit()
    d["hlo_predicted_from_parental"] = m.predict(X)
    d["hlo_residual"] = d["hlo_sec"] - d["hlo_predicted_from_parental"]

    print("=" * 72)
    print("HLO decomposition: home-niche-predicted vs residual (school quality "
    ...
    print()
    print("  Top 10 POSITIVE residuals (HLO higher than the home niche predicts —")
    ...
    print()
    print("  Top 10 NEGATIVE residuals (HLO lower than the home niche predicts —")
    ...
    ax.set_xlabel("Lower-secondary completion at 1990 "
                  "(parents' education 28 years ago, %)",
                  fontsize=11)
    ...
    if FOR_WEB:
        ax.set_title(
            ...
            "of their parents'\neducation 28 years ago — the paper's home-niche "
            "mechanism in action\n"
    ...
    out_path = os.path.join(PAPER_FIGS if not FOR_WEB else WEB_FIGS,
                            "hlo_vs_parental_education_1990.png")
    fig.savefig(out_path, dpi=180, bbox_inches="tight")
    print(f"Saved: {out_path}")

    write_checkin("hlo_is_parental_education.json", {
        "numbers": {
            # Test 1: HLO ~ lsec_1990
            "t1_n": int(len(d1)),
