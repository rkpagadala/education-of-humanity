...
peer pools under five matching regimes:
  ...
  D. depth           — MYS15+ ±0.5 AND MYS15+ at t-28 ±0.5 (generational
                       depth: matches the parents' substrate too)
  E. full_substrate  — MYS15+ ±0.5 AND MYS15+ at t-28 ±0.5 AND % no-edu ±5pp
                       AND F-M MYS gap ±0.5 (maximal substrate snapshot+lag28)
  F. depth_50        — MYS15+ ±0.5 AND MYS15+ at t-28 ±0.5 AND MYS15+ at t-56
                       ±0.5 (three-generation depth: parents AND grandparents)
  G. full_50         — D + lag56 + %no-edu + F-M gap (maximal substrate;
...
does not register any number with verify_the_long_childhood.py.
...
from _shared import REPO_ROOT, NAME_MAP, REGIONS, load_wb, write_checkin, LAG_GENERATION
...
def build_feature_panel(annual):
    """Education features only, 1950-2015. Used for 28-year-lagged lookups
...
def add_lag_column(panel, feature_panel, n_years=LAG_GENERATION):
...
def match_depth(panel, target_row, country):
    if pd.isna(target_row.get("mys15p_lag28")):
        return panel.iloc[0:0]
    sub = panel.dropna(subset=["mys15p", "mys15p_lag28"])
    return sub[
        (abs(sub["mys15p"] - target_row["mys15p"]) <= MYS_BAND) &
        (abs(sub["mys15p_lag28"] - target_row["mys15p_lag28"]) <= MYS_BAND) &
        (sub["country"] != country)
    ]
...
def match_full_substrate(panel, target_row, country):
    if pd.isna(target_row.get("mys15p_lag28")):
        return panel.iloc[0:0]
    sub = panel.dropna(subset=["mys15p", "mys15p_lag28", "pct_no_edu", "gap_f_m"])
    return sub[
        (abs(sub["mys15p"] - target_row["mys15p"]) <= MYS_BAND) &
        (abs(sub["mys15p_lag28"] - target_row["mys15p_lag28"]) <= MYS_BAND) &
        (abs(sub["pct_no_edu"] - target_row["pct_no_edu"]) <= NO_EDU_BAND) &
...
def match_depth_50(panel, target_row, country):
    if pd.isna(target_row.get("mys15p_lag28")) or pd.isna(target_row.get("mys15p_lag56")):
        return panel.iloc[0:0]
    sub = panel.dropna(subset=["mys15p", "mys15p_lag28", "mys15p_lag56"])
    return sub[
        (abs(sub["mys15p"] - target_row["mys15p"]) <= MYS_BAND) &
        (abs(sub["mys15p_lag28"] - target_row["mys15p_lag28"]) <= MYS_BAND) &
        (abs(sub["mys15p_lag56"] - target_row["mys15p_lag56"]) <= MYS_BAND) &
        (sub["country"] != country)
    ]
...
def match_full_50(panel, target_row, country):
    ...
    sub = panel.dropna(subset=[
        "mys15p", "mys15p_lag28", "mys15p_lag56", "pct_no_edu", "gap_f_m"
    ])
    return sub[
        (abs(sub["mys15p"] - target_row["mys15p"]) <= MYS_BAND) &
        (abs(sub["mys15p_lag28"] - target_row["mys15p_lag28"]) <= MYS_BAND) &
...
REGIMES = [
    ...
    ("depth", "MYS15+ ±0.5 AND MYS15+(t-28) ±0.5", match_depth),
    ("full_substrate",
     "MYS15+ ±0.5 AND MYS15+(t-28) ±0.5 AND %no-edu ±5pp AND F-M gap ±0.5",
     match_full_substrate),
    ("depth_50",
     "MYS15+ ±0.5 AND MYS15+(t-28) ±0.5 AND MYS15+(t-56) ±0.5",
     match_depth_50),
    ("full_50",
     "MYS15+ ±0.5 AND MYS15+(t-28) ±0.5 AND MYS15+(t-56) ±0.5 AND %no-edu ±5pp AND F-M gap ±0.5",
     match_full_50),
]
...
def main():
    ...
    panel = add_lag_column(panel, feature_panel, n_years=LAG_GENERATION)
    panel = add_lag_column(panel, feature_panel, n_years=2*LAG_GENERATION)
    print(f"  Panel: {len(panel):,} obs, {panel['country'].nunique()} countries")
    print(f"  Rows with mys15p_lag28 available: {panel['mys15p_lag28'].notna().sum():,}")
    print(f"  Rows with mys15p_lag56 available: {panel['mys15p_lag56'].notna().sum():,}")
