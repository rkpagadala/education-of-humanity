...
def china_young_lsec_count(year=2010):
    """
    Count of young Chinese (age 15-29) who have completed at least lower
    secondary (9 years) schooling, in millions, for the given year.

    Source: WCDE v3 pop_both.csv (population by age × education level).
    Sums Lower Secondary + Upper Secondary + Post Secondary across the
    15--19, 20--24, 25--29 age bands, both sexes, scenario 2. WCDE pop is
    in thousands; divide by 1000 for millions.
    """
    pop = pd.read_csv(os.path.join(WCDE_RAW, "pop_both.csv"))
    bands = ["15--19", "20--24", "25--29"]
    lsec_plus = ["Lower Secondary", "Upper Secondary", "Post Secondary"]
    mask = (
        (pop["name"] == "China") &
        (pop["year"] == year) &
        (pop["scenario"] == 2) &
        (pop["age"].isin(bands)) &
        (pop["education"].isin(lsec_plus))
    )
    return pop[mask]["pop"].sum() / 1000.0  # thousands -> millions
...
def save_checkin(result, break_results, panel):
    ...
    checkin_data = {
        ...
        "key_data_points": {
            ...
            "china_young_lsec_millions_2010": round(china_young_lsec_count(2010), 1),
