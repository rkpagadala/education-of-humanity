...
Life-history label (§the-generational-lag):
  Predictor: WCDE 20-24 cohort completion at year T_x (first year a
  country crossed completion threshold x).
  Outcome: development phenotype (TFR<3.65 AND LE>69.8) by year
  T_x + 28.
  Generational relation: one generational step (~28 yr). The 28-yr
  window is the lower bound — the cohort that crossed at T_x is
  age 45-49 at T_x+28 (mostly past reproduction); their daughters
  are the ones bearing children at T_x+28.
  Use: identification of the sufficiency threshold by forward
  prediction (does crossing x% predict development crossing within
  one generation?).

Forward-lag test of the home-niche mechanism:
  ...
  phenotype (TFR < 3.65 AND LE > 69.8) by T_x + 28.
...
from _shared import (
    ...
    load_wide_indicator, REGIONS, WB_TO_WCDE, write_checkin, LAG_GENERATION,
)
...
def main():
    ...
    print()
    print(f"{'Threshold x':<12}  {'n_reached':>10}  {'crossed by T+28':>18}  "
