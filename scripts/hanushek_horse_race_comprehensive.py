...
Hanushek-style quality measures, with four refinements:
  ...
  1. Outcome-specific educational depth (all lower-secondary or above
     since 2026-05-26 — canonical lag for TFR is lag 0, where lower-sec
     dominates primary in within-country panel R² per by_level_lag0.json):
       - TFR       ← lower-secondary completion (mother's own depth
                                          at contemporaneous lag)
...
def load_population():
    """Country total population at 2015, in thousands.

    Read from preprocessed `wcde/data/processed/population_2015.csv` so the
    public replication repo does not need the 91 MB raw `pop_both.csv`.
    The processed file is generated by `wcde/scripts/build_population_2015.py`
    and is a row-per-country aggregate (scenario=2, year=2015, summed across
    age/sex/education).
    """
    ...
    for _, r in pop.iterrows():
        c = r['country']
        wdi = WCDE_TO_WDI.get(c, c)
        out[wdi] = float(r['population_thousands'])
    return out
...
def main():
    ...
    # Race 1: TFR ← lower-secondary
    # (Switched from primary 2026-05-26: canonical lag for TFR per
    # _shared.py is lag 0, where lower-sec dominates over primary in
    # within-country panel R² — see by_level_lag0.json.)
    tfr_res = run_race(panel, "tfr_2015",
             "TFR at 2015  (lower-sec-driven hypothesis)",
