...
PAPER ANCHOR — Chapter 9 §Education vs GDP at Population Scale, §GDP residualised
  Question: once log GDP is residualised against education
            (Frisch-Waugh-Lovell), does it retain any independent predictive
            power for life expectancy, fertility, child education, or U5MR?
  Produces: Table~\\ref{tab:residualisation} (paper §gdp-has-no-independent-effect)
            and Figure~\\ref{fig:edu-vs-resid-gdp}. Headline:
            residualised-GDP R^2 <= 0.019 across all four outcomes, all p >= 0.11.
  Checkin:  checkin/education_vs_gdp_lifeexpectancy.json (and the companion
            JSONs for the other outcomes in this directory).
...
Step 2: Regress LE(T+LAG_LE) on these residuals with country FE.
        R² = GDP's independent predictive power for future LE,
        after removing education's contribution to GDP.

Compare:
  - Edu R²:          education → LE(T+LAG_LE)
  - Raw GDP R²:      log_GDP → LE(T+LAG_LE)
  - Residual GDP R²: GDP_residual → LE(T+LAG_LE)
...
from _shared import (
    ...
    print_summary, write_checkin, CHECKIN, fmt_r2, LAG_LE,
)
...
LAG = LAG_LE   # LE one generation forward (cohort longevity; reverse-causality discipline)
...
for level_name, col_name in EDU_LEVELS.items():
    ...
    # Build panel
    panel = build_panel(edu_annual, le_raw, gdp_raw, T_YEARS, LAG, "le_tp12")
    ...
    level_results = run_residualized_sweep(
        panel, entry_years, "le_tp12", CEILINGS, label="LE")
...
checkin = {
    "method": (
        ...
        "by education), (2) use residuals to predict LE(T+12). Compares "
        "education R², raw GDP R², and residualized GDP R². "
        "Entry-cohort with ceiling. T=1960-1990, lag=12 (childrearing window). "
