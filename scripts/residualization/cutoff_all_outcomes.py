...
four outcomes:
  1. Life expectancy at T+28 (LE)
  2. Total fertility rate at T+5 (TFR)
  3. Log under-5 mortality at T+12 (log U5MR)
  4. Child education at T+28 (same country, intergenerational)
...
Matches scripts/residualization/education_predicts_le.py:
  ...
  - Lag: outcome-specific. TFR uses LAG_TFR=5 (biological parent timing —
    the T cohort is age 25-29 at T+5, the reproductive peak). log U5MR uses
    LAG_CHILDREARING=12 (childrearing window). LE and child education use
    LAG_GENERATION=28 (time-to-agency / edu→edu).
  ...
  - Child education: lower secondary completion at T+28 in the same country
...
T predict the outcome at T+28 once we remove country fixed effects?"
...
# log U5MR uses LAG_CHILDREARING=12 (childrearing window); LE and child
# education use LAG_GENERATION=28 (time-to-agency / edu→edu transmission).
OUTCOME_LAG = {
    "le":        LAG_GENERATION,
    "tfr":       LAG_TFR,
    "log_u5mr":  LAG_CHILDREARING,
    "child_edu": LAG_GENERATION,
}
OUTCOME_LABELS = {
    "le":        f"Life expectancy (T+{LAG_GENERATION})",
    "tfr":       f"Fertility rate (T+{LAG_TFR})",
    "log_u5mr":  f"log Under-5 mortality (T+{LAG_CHILDREARING})",
    "child_edu": f"Child education (T+{LAG_GENERATION})",
}
...
# at its own lag. TFR is read at T+LAG_TFR (=5); LE, log U5MR, and child
# education at T+LAG_GENERATION (=28). Child-education outcome uses WCDE
# lower-sec at T+28 in the same country (the intergenerational measure).

print(f"\nBuilding panel (T={T_YEARS}, lags={OUTCOME_LAG})...")

# Precompute a country-indexed education series for fast T+lag lookup.
# Interpolate to annual: the child-education outcome is read at T+LAG_GENERATION=28,
# which is off the 5-year WCDE grid, so an exact-year lookup would drop every
# child_edu observation. Grid-year values are preserved by linear interpolation.
edu_lookup = {}
for c, grp in edu.groupby("country"):
    s = grp.dropna(subset=["lower_sec"]).set_index("year")["lower_sec"].sort_index()
    if len(s) >= 2:
        full = range(int(s.index.min()), int(s.index.max()) + 1)
        s = s.reindex(full).interpolate(method="linear")
    edu_lookup[c] = s
...
for c in sorted(edu["country"].unique()):
    ...
    for t in T_YEARS:
        ...
        # Outcome-specific lags
        t_le  = t + OUTCOME_LAG["le"]
        t_tfr = t + OUTCOME_LAG["tfr"]        # = t + LAG_TFR
        t_u5  = t + OUTCOME_LAG["log_u5mr"]
        t_ced = t + OUTCOME_LAG["child_edu"]

        le_val   = get_wb_val(le_raw,   c, t_le)
        tfr_val  = get_wb_val(tfr_raw,  c, t_tfr)
        u5_val   = get_wb_val(u5mr_raw, c, t_u5)
        log_u5   = np.log(u5_val) if (not np.isnan(u5_val) and u5_val > 0) else np.nan
        child_ed = (edu_c.loc[t_ced] if t_ced in edu_c.index else np.nan)
        ...
        rows.append({
            ...
            "log_gdp_t": log_gdp,
            "le": le_val,
            "tfr": tfr_val,         # outcome read at T+LAG_TFR=5
            "log_u5mr": log_u5,
            "child_edu": child_ed,
        })
...
for o in OUTCOMES:
    print(f"    {o} (lag={OUTCOME_LAG[o]}): {panel[o].notna().sum()} non-null obs")


# ── Run all cutoffs × all outcomes ───────────────────────────────────
print("\n" + "=" * 95)
print("Education(T) vs log_GDP(T) predicting outcomes(T+lag) — Country FE (within R²)")
print(f"T = {T_YEARS} | Lags = TFR:{LAG_TFR}y, U5MR:{LAG_CHILDREARING}y, "
      f"LE/childEdu:{LAG_GENERATION}y")
print("=" * 95)
...
write_checkin("cutoff_all_outcomes.json", {
    "method": (
        ...
        f"T = {T_YEARS}. Outcome-specific lags: TFR at T+LAG_TFR={LAG_TFR} "
        f"(biological parent-of-newborn timing); log U5MR at "
        f"T+LAG_CHILDREARING={LAG_CHILDREARING} (childrearing window); LE and "
        f"child education at T+LAG_GENERATION={LAG_GENERATION} (time-to-agency / "
        "edu→edu transmission)."
    ),
    ...
    "outcome_variables": {
        "le": f"life expectancy at birth (WDI SP.DYN.LE00.IN), at T+{LAG_GENERATION}",
        "tfr": f"total fertility rate (WDI SP.DYN.TFRT.IN), at T+{LAG_TFR}",
        "log_u5mr": f"log under-5 mortality per 1000 (WDI), at T+{LAG_CHILDREARING}",
        "child_edu": f"lower secondary completion at T+{LAG_GENERATION}, same country (WCDE v3)",
    },
