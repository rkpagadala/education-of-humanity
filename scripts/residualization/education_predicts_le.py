...
how long its people will live 28 years from now? And how does that compare
...
For each country-year pair (T), we ask:
  - Does education(T) predict life expectancy(T+28)?
  - Does GDP(T) predict life expectancy(T+28)?
...
28 years later? When its GDP rose, did life expectancy follow?

The 28-year lag is one generation: a woman educated at age 15-18 has
...
- Outcome: life expectancy at T+28 (= 2003, 2008, 2013, 2018)
...
WHY T=1975-1990?
  WCDE education data starts at 1950. With a 28-year lag, outcomes start
  at 1978. But GDP data before 1960 is sparse, so the effective GDP panel
  starts at T=1960 (outcome 1988). We use T=1975-1990 to maximise overlap
...
Formally: for country i at time t,
  ...
  le_demeaned(i,t)  = le(i,t+28) - mean(le(i,:+28))
...
for c in sorted(edu["country"].unique()):
    ...
    for t in T_YEARS:
        ...
        if np.isnan(low) or np.isnan(le_tp28):
            continue
        rows.append({
            "country": c,
            "t": t,
...
print("\n" + "=" * 85)
print("Education(T) vs GDP(T) predicting Life Expectancy(T+28)")
print("Country fixed effects | T = 1975, 1980, 1985, 1990 | Lag = 28 years")
print("=" * 85)
...
for cutoff in [10, 20, 30, 40, 50, 60, 70, 80, 90, None]:
    ...
    r2_e, n_e, c_e = fe_r2("low_t", "le_tp28", sub)
    r2_g, n_g, c_g = fe_r2("log_gdp_t", "le_tp28", sub)
...
write_checkin("edu_vs_gdp_predicts_le.json", {
    "method": "Country FE (demeaned), T=1975/1980/1985/1990, outcome=LE(T+28)",
