...
T = 1960-1990 (5yr); lag = LAG_TFR (5y) — biological parent timing.
The parent cohort age 20-24 at T is age 25-29 at T+5 (reproductive
peak); period TFR at T+5 reflects that cohort's own fertility.
"""
...
for level_name, col_name in EDU_LEVELS.items():
    ...
    # Build panel — outcome at T+LAG_TFR=T+5 (biological parent timing).
    panel = build_panel(edu_annual, tfr_raw, gdp_raw, T_YEARS, LAG, "tfr_tp5")
    ...
    level_results = run_residualized_sweep(
        panel, entry_years, "tfr_tp5", CEILINGS, label="TFR")
...
checkin = {
    "method": (
        ...
        "by education), (2) use residuals to predict TFR(T+LAG_TFR). Compares "
        "education R², raw GDP R², and residualized GDP R². "
        "Entry-cohort with ceiling. T=1960-1990, lag=LAG_TFR=5 (parent cohort "
        "age 20-24 at T is age 25-29 at T+5, the reproductive peak). "
        "Three education levels: primary, lower secondary, upper secondary."
    ),
    "levels": all_results,
    "numbers": {
