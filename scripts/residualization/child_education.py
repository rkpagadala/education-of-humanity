...
Parent education(T) → Child education(T+28): the home-niche mechanism.
GDP(T) → Child education(T+28): the income-led hypothesis.
Residualized GDP(T) → Child education(T+28): GDP's independent effect.
...
from _shared import (
    ...
    print_summary, write_checkin, fmt_r2, LAG_GENERATION,
)
...
for level_name, col_name in EDU_LEVELS.items():
    print(f"\n{'#' * 90}")
    print(f"# {level_name.upper().replace('_', ' ')}  →  CHILD EDUCATION(T+28)")
    print(f"{'#' * 90}")
...
for ceiling in [60, 90]:
    print(f"\n{'=' * 90}")
    print(f"CROSS-LEVEL: entry=10%, ceiling={ceiling}%  →  Child Education(T+28)")
    print(f"{'=' * 90}")
...
write_checkin("edu_vs_gdp_child_edu_residualized.json", {
    "method": "Country FE, residualized GDP. Parent edu(T) → child edu(T+28). Entry-cohort + ceiling. T=1960-1990, lag=28.",
    "levels": all_results,
