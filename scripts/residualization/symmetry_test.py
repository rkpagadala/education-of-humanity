...
Outcome-specific lags: TFR uses LAG_TFR=5 (biological parent-of-newborn
timing); life expectancy uses LAG_GENERATION=28 (time-to-agency).
...
OUTCOMES = {
    "life_expectancy": ("life_expectancy_years.csv", "le_tp28"),
    "tfr": ("children_per_woman_total_fertility.csv", "tfr_tp5"),
}
# Outcome → lag. TFR uses LAG_TFR=5 (biological parent-of-newborn timing);
# life expectancy uses LAG_GENERATION=28 (time-to-agency).
OUTCOME_LAG = {
    "life_expectancy": LAG_GENERATION,
    "tfr":             LAG_TFR,
}
...
for outcome_name, (csv, ycol) in OUTCOMES.items():
    ...
    for level_name, col_name in EDU_LEVELS.items():
        ...
        panel = build_panel(edu_annual, out_raw, gdp_raw, T_YEARS,
                            OUTCOME_LAG[outcome_name], ycol)
...
write_checkin(
    "residualization_symmetry_test.json",
    {
        "method": (
            ...
            "parent-of-newborn timing); life expectancy at "
            f"T+LAG_GENERATION={LAG_GENERATION} (time-to-agency)."
        ),
