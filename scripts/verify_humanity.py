...
reg("T1-countries",  185,    "checkin", ("panel_full_fe.json", "numbers.panel_countries"),
    [(ABSTRACT, 147), (THE_EVIDENCE, None), (DATA_SEC, 3), (APPENDIX_ROBUST, None),
...
reg("GDP-Philippines-1960", 1124, "wdi", ("gdp", "Philippines", 1960), [(KOREA_PHILIPPINES, None)], tol=5)
reg("GDP-Thailand-1960",    592, "wdi", ("gdp", "Thailand", 1960), [(KOREA_PHILIPPINES, 19)], tol=5)
reg("GDP-Indonesia-1960",   598, "wdi", ("gdp", "Indonesia", 1960), [(KOREA_PHILIPPINES, 20)], tol=5)
reg("GDP-India-1960",       313, "wdi", ("gdp", "India", 1960), [(KOREA_PHILIPPINES, 20)], tol=5)
reg("GDP-China-1960",       241, "wdi", ("gdp", "China", 1960), [(KOREA_PHILIPPINES, 20)], tol=5)
...
reg("Phil-TFR-2022",        1.9,  "wdi", ("tfr", "Philippines", 2022),
    [(KOREA_PHILIPPINES, None)], tol=0.1)
reg("Phil-LE-2022",         69.5, "wdi", ("le", "Philippines", 2022),
    [(KOREA_PHILIPPINES, None)], tol=0.3)
...
reg("TFR-threshold-intro",   3.65,  "wdi", ("tfr", "USA", 1960), [DEF_DEV], tol=0.01)
reg("LE-threshold-intro",    69.8,  "wdi", ("le", "USA", 1960), [DEF_DEV], tol=0.05)
...
reg("Korea-ppyr-sec",        2.13,   "derived", "(Korea-1985 - Korea-1955) / 30", [SEN_CASES, KOREA_PHILIPPINES, POLICY], tol=0.1)
# REMOVED from paper
...
reg("IV-inst-F",                   9.61, "checkin",
    ...
    [(COLONIAL, None), (INST_CHAL, None), (APPX_COL_IV, None)], tol=0.5)
reg("IV-edu-coef",                0.043, "checkin",
    ("iv_2sls_colonial_icrg.json", "gdp_edu_2sls_coef"),
    [(COLONIAL, None), (INST_CHAL, None), (APPX_COL_IV, None)], tol=0.005)
reg("IV-edu-t",                   2.56,  "checkin",
    ("iv_2sls_colonial_icrg.json", "gdp_edu_2sls_t"),
    [(COLONIAL, None), (INST_CHAL, None), (APPX_COL_IV, None)], tol=0.1)
...
reg("IV-avexpr-edu-F",           37.13, "checkin",
    ...
    [(COLONIAL, None), (INST_CHAL, None), (APPX_COL_IV, None), (CAUSAL_BAD, None)],
    tol=0.5)
...
reg("IV-avexpr-edu-t",            6.74, "checkin",
    ...
    [(COLONIAL, None), (INST_CHAL, None), (APPX_COL_IV, None)], tol=0.1)
# AJR base sample size constants
...
reg("AJR-merged-n",              61, "const",
    ...
    [(COLONIAL, None), (INST_CHAL, None), (APPX_COL_IV, None)], tol=0)
...
# --- POLICY section: SDG 4 implied-pace inversion ---
reg("SDG4-median-pace", 8.3, "checkin",
    ("sdg4_implied_pace.json", "implied_pace_pp_per_yr.median"),
    [POLICY], tol=0.1)
reg("SDG4-korea-record", 2.13, "checkin",
    ("sdg4_implied_pace.json", "korea_record_pp_per_yr"),
    [POLICY], tol=0.01)
reg("SDG4-years-remaining", 5, "checkin",
    ("sdg4_implied_pace.json", "years_remaining"),
    [POLICY], tol=0)
...
reg("Korea-income-at-expansion", 1038, "derived",
    ...
    [(SEN_CASES, None), (KOREA_PHILIPPINES, None)], tol=50)
...
reg("Nepal-income-at-crossing", 1114, "wdi",
    ("gdp", "Nepal", 2022),
    [("four-further-cases", None)], tol=50)
...
reg("ColA6-logem4-r2",   0.544, "checkin",
    ...
    [("the-institutional-challenge", None)], tol=0.005)
reg("ColA6-edu1900-r2",  0.248, "checkin",
    ("ajr_n61_deep_dive.json", "univar_r2_log_gdp_2020.edu_1900_cohort_lsec.r2"),
    [("the-institutional-challenge", None)], tol=0.005)
reg("ColA6-religion-r2", 0.055, "checkin",
    ("ajr_n61_deep_dive.json", "univar_r2_log_gdp_2020.protestant.r2"),
    [("the-institutional-challenge", None)], tol=0.005)
reg("ColA6-edu-plus-religion-r2", 0.521, "checkin",
    ("ajr_n61_deep_dive.json", "univar_r2_log_gdp_2020.edu_plus_religion.r2"),
    [("the-institutional-challenge", None)], tol=0.005)

# --- the-institutional-challenge: Table A7 2SLS contest (avexpr) ---
...
def main():
    ...
    STRUCTURAL_NUMBERS = {
        ...
        1700,           # Europe-1700 sail-era reference (§the-asymmetry-reverses)
        1800,           # post-1800 Preston Curve reference
        1945,           # post-1945 institutional architecture (§the-four-radii-reverse)
        ...
        if isinstance(val, (float, int)):
            ...
            for fmt in [".4f", ".3f", ".2f", ".1f", ".0f", "g"]:
                ...
                if fv < 0:
                    ...
                    pats.add(f"-{format(abs(fv), fmt)}")
            # Suffix forms for large round values: "1.5 million", "one~million", etc.
            # Pick the single largest applicable scale so we don't emit "1000 thousand".
            afv = abs(fv)
            scale = None
